...
| commanded **position** and **velocity** streams, every kind | bit-identical | so C-HAND-1's stroke-busy window, the throw's event-delay floor, `_PRIME_INFLIGHT_S`, and `tools/probes/hand_stroke_timeline.py`'s whole verdict model stay valid **without moving**. (That floor was the constant `MIN_THROW_EVENT_DELAY_S = 1.0 s` when this row was written; on 2026-08-22 it became `hand_stroke.min_throw_event_delay_s(v_throw)`, derived from the same stroke geometry. The row's claim is unaffected — the streams this change leaves bit-identical are exactly the ones that function reads.) |
