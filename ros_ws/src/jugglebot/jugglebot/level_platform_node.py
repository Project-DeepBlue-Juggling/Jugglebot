...
class LevelPlatformNode(Node):
    def __init__(self):
        super().__init__('level_platform_node')

        # Set up a service to trigger closing the node
        self.service = self.create_service(Trigger, 'end_session', self.end_session)

        # Set up a flag to indicate whether the node should be shut down
        ...
        # Set up an action client to conduct the auto-levelling process
    ...
    async def execute_auto_calibrate_callback(self, goal_handle):
        ...
        try:
            # First set conservative movement speeds
            self.set_legs_vel_limit(leg_vel_limit=2.5)
            
            # Now move to the calibration pose
            self.move_to_calibration_pose_and_await_arrival()
            time.sleep(0.1) # Add a short break just to be sure that the motors have stopped moving
            ...
            feedback = LevelPlatform.Feedback()
            ...
            # Convert the tilt readings to a quaternion
            tilt_quat = self.tilt_to_quat(tiltX, tiltY)

            goal_handle.succeed()

            # Publish this tilt reading as the result
            result = LevelPlatform.Result()
            result.success = True
            result.tilt_xy = [tiltX, tiltY]
            result.tilt_quat = Quaternion(x=tilt_quat.x, y=tilt_quat.y, z=tilt_quat.z, w=tilt_quat.w)

            # Return the leg velocity limit to 50 rev/s
    ...
    def move_to_calibration_pose_and_await_arrival(self):
        ...
        message.pose_stamped.pose.orientation.x = 0.0
        message.pose_stamped.pose.orientation.y = 0.0
        message.pose_stamped.pose.orientation.z = 0.0
        message.pose_stamped.pose.orientation.w = 1.0

        # Set the time stamp
        message.pose_stamped.header.stamp = self.get_clock().now().to_msg()
    ...
    def process_tilt_reading(self, future):
        ...
        if response is not None:
            self.tilt_readings = future.result().tilt_xy
        else:
    ...
    def convert_tilt_to_quat_and_publish(self, tiltX, tiltY):
        ...
        tilt_offset_quat = self.tilt_to_quat(tiltX, tiltY)
    ...
    def tilt_to_quat(self, tiltX, tiltY):
        '''Convert tilt readings to a quaternion offset to apply to the platform orientation to level it'''
    ...
    def end_session(self, request, response):
        ...
        response.message = "Shutting down platform alignment node..."
        self.get_logger().info("Shutting down platform alignment node...")
        return response


def main(args=None):
    rclpy.init(args=args)
    node = LevelPlatformNode()
