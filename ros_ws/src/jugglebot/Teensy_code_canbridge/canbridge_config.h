...
// ── Hand-lane (axis 6) guard constants — unified-7dof FW 17 ──────────────────
//  OWNER-SIGNED 2026-08-30 (Phase 0 Decision 4,
//  logbook/2026-08-30-unified-7dof-planner-phase0-probes.md). The leg constants
//  above are LEG numbers and must NEVER be applied to axis 6: the hand runs
//  13× leg speed, so the leg values are either dead weight (a deviation bound
//  the stroke can't reach) or a stall (a 0.10 rev lead clamp that clips every
//  peak-speed throw — the S1/S2 silent-stall failure mode at 13× the speed).
//
//  MAX_DEVIATION_HAND_REV (2.5 rev = 79.1 mm): VELOCITY-COMPENSATED BOTH SIDES
//  — the residual is (interpolated command now) − (fb + fb_vel·age), computed
//  in the 500 Hz interp tick; the 10 Hz fault task latches the E-STOP off the
//  tick's verdict (fault_machine.cpp). A static worst-lag bound was ruled out
//  by arithmetic: honest apparent deviation at 221 rev/s sums to 8.6–11.3 rev
//  against a 10.701 rev stroke (FW 18, 2026-09-08; was 10.8) — a guard that
//  cannot fire before the hard stop.
//  Ships OBSERVE-FIRST (leg_interp.cpp `hand7 arm` console switch, boot
//  default observe): report the max residual at the first sitting, arm the
//  trip at the second — an observe-only guard left forever is a guard that
//  does not exist (Phase 0 Open Question 4 names the second sitting's step).
constexpr float    MAX_DEVIATION_HAND_REV = 2.5f;
//  MAX_LEAD_HAND_REV (2.0 rev = 63.3 mm): the lead clamp anchor is the
//  AGE-EXTRAPOLATED encoder (fb + vel·age) — freshness-aware, because a
//  freshness-blind clamp needs 5.0 rev (46 % of stroke) to avoid clipping
//  legitimate commands, and the naive leg-time-equivalent (1.3 rev) sits below
//  the 3.5 rev staleness term and clips EVERY peak-speed throw. The REQUIRED
//  lead-duty counter lives beside the clamp (interp_hand_lead_clamp_ticks —
//  [hand7] console + lead_clamp_mask bit 6): non-zero duty during a throw is a
//  hard-abort-the-sitting signal (the FW 14 clamp-duty-0 precedent).
constexpr float    MAX_LEAD_HAND_REV      = 2.0f;
//  HAND_VELFF_LIMIT_RPS (300): the coupled fifth constant. The legs'
//  LEAD_CLAMP_VELFF_LIMIT_RPS is 3.5 — a mechanical port would be a 51×
//  feedforward cut, i.e. the velocity FF silently ceasing to exist on the
//  fastest axis in the machine. 300 = the owner-signed hand velocity CEILING
//  (above any planned hand speed, below the 327.67 rev/s int16 wire
//  saturation), tied to the config chain rather than a literal.
constexpr float    HAND_VELFF_LIMIT_RPS   = TrajOp::HAND_VEL_CEILING_RPS;   // 300
//  Hand overspeed E-STOP bound (fault_machine.cpp): ~1.15× the 300 rev/s
//  ceiling = 345. The planner cap (200/300) is otherwise the only practical
//  hand overspeed guard — the leg loop correctly excludes axis 6 and the next
//  backstop is the ODrive's own 1200 rev/s trip.
constexpr float    HAND_MAX_MOTOR_VEL_RPS = TrajOp::HAND_VEL_CEILING_RPS * 1.15f;   // 345
//  (HAND_SETTLE_BAND_REV / HAND_SOURCE_SETTLE_VEL_RPS lived here as the
//  hand_source switch's settle gate. Both went with the latch at FW 21
//  (skill-stack R1): there is no switch to gate, so the firmware has no
//  consumer, and the host config chain is now the SINGLE source of the settle
//  band (INVARIANTS.md item 9 — no timing/geometry twin).
...
constexpr uint32_t SETPOINT_STALENESS_US = 250000u;   // 0.25 s → E-STOP (link-fault trigger)
constexpr uint32_t MOTOR_FB_STALENESS_US = 150000u;  // 0.15 s → suppress commands
// AXIS SILENCE (FW 23) — the FATAL predicate. A PRESENT leg that has sent NO
// frame OF ANY KIND on the Jugglebot bus for this long is silent → CAN_BUS_DOWN
// (output suppressed, deferred stow armed). Renamed from CAN_HEARTBEAT_TIMEOUT_US
// with the same 2.0 s value: until FW 23 the fatal predicate read ONLY the 10 Hz
// ODrive heartbeat, so 20 consecutive lost heartbeats stowed the machine while the
// SAME leg's 100 Hz encoder stream kept arriving (2026-09-15: two CAN_BUS_DOWN
// trips, 0 CAN errors, ring leak 0, worst encoder age 95 ms). Liveness is now the
// union of every cyclic stream the node emits (AxisState::last_rx_us), which is
// ~272 frames/s/leg instead of 10 — the same 2.0 s window now needs ~544 lost
// frames, not 20.
constexpr uint32_t CAN_AXIS_SILENCE_TIMEOUT_US = 2000000u;  // 2.0 s → fatal_can_error
// HEARTBEAT staleness (FW 23) — DIAGNOSTIC ONLY, never a fault. Deliberately
// SHORTER than the silence timeout (5 lost heartbeats, not 20) so the
// HeartbeatT2J HB_STALE_MASK is SENSITIVE to the load-gated per-leg frame loss
// that plans/active/leg-bus-frame-drops.md tracks: the dropout is the thing to
// report, and reporting it must not be gated on the threshold that stows.
constexpr uint32_t CAN_HEARTBEAT_STALE_US = 500000u;     // 0.5 s → HB_STALE_MASK bit (report-only)
...
// CAN_AXIS_SILENCE_TIMEOUT_US staleness term — so the gate is never slower to react
...
// ── FW 22 scheduled (stamped) playback — hand C2 spec 2026-09-14 ─────────────
// A HAS_SCHED setpoint is PLAYED at its t_origin_us stamp, not at arrival. It
// needs the full knot set (HAS_U1|HAS_U2|HAS_V1|HAS_V2) and a synced bridge
// clock; otherwise it is counted (sched_demoted) and played as a legacy frame.
constexpr uint32_t SP_FLAG_HAS_V2       = 0x10u;
constexpr uint32_t SP_FLAG_HAS_SCHED    = 0x20u;
constexpr uint32_t SCHED_REQUIRED_FLAGS = 0x1Bu;   // HAS_U1|HAS_U2|HAS_V1|HAS_V2
// Pending queue depth. At 40 Hz with a 1-knot emit lead the queue holds <= 2;
// 4 absorbs a late burst. Overflow drops the OLDEST stamp (counted).
constexpr uint8_t  SCHED_QUEUE_LEN      = 4;
// A stamp further than this ahead of arrival is a clock-domain confusion, not a
// lead: refuse it (counted, and the frame does NOT feed the staleness clock).
constexpr int64_t  SCHED_MAX_FUTURE_US  = 250000;
// Promotion continuity tolerances (|outgoing - incoming| at the promotion tick).
// A consistent knot-aligned source gives 0 up to float32 knot quantisation plus
// a few-us stamp jitter: hand 200 rev/s x 5 us = 1e-3 rev, jerk 1.4e5 x 5 us =
// 0.7 rev/s^2. Over-tolerance promotions are COUNTED; out of a stop/hold while
// armed they are REFUSED (the latched-hold rule, leg_interp.cpp).
constexpr float    SCHED_PROMO_TOL_POS_LEG_REV   = 0.002f;
constexpr float    SCHED_PROMO_TOL_VEL_LEG_RPS   = 0.1f;
constexpr float    SCHED_PROMO_TOL_ACC_LEG_RPS2  = 10.0f;
constexpr float    SCHED_PROMO_TOL_POS_HAND_REV  = 0.005f;
constexpr float    SCHED_PROMO_TOL_VEL_HAND_RPS  = 0.5f;
constexpr float    SCHED_PROMO_TOL_ACC_HAND_RPS2 = 50.0f;
// RESUME tolerances — the accept/refuse decision out of a stop/hold while armed,
// deliberately looser than the diagnostic tolerances above. With the host emit
// lead at 0 knots a frame lands 1-15 ms after its stamp, so a frame more than
// 25 ms late exhausts the previous cover and meets a stop that is only a tick or
// two old. The stop and the plan leave the shared entry state with different
// jerk, so after δ the gap is ~Δj·δ in accel, Δj·δ²/2 in vel, Δj·δ³/6 in pos
// (hand Δj up to ~3.5e5 rev/s³: δ = 2 ms → 700 rev/s², 0.7 rev/s, 5e-4 rev).
// Refusing that would latch-hold (and E-STOP) a healthy stream on one hiccup; a
// deeper stop (δ ≳ 3 ms hand, ≳ 2 ms legs) is refused — the latched hold.
constexpr float    SCHED_RESUME_TOL_POS_LEG_REV   = 0.005f;
constexpr float    SCHED_RESUME_TOL_VEL_LEG_RPS   = 0.3f;
constexpr float    SCHED_RESUME_TOL_ACC_LEG_RPS2  = 60.0f;
constexpr float    SCHED_RESUME_TOL_POS_HAND_REV  = 0.05f;
constexpr float    SCHED_RESUME_TOL_VEL_HAND_RPS  = 8.0f;
constexpr float    SCHED_RESUME_TOL_ACC_HAND_RPS2 = 1000.0f;
// GRACE before the C2 stop. At the host's emit lead of 0 knots EVERY emitter
// overrun drops a knot, and the next frame lands 1-15 ms after its stamp — past
// the previous frame's 2T cover. Stopping at 2T would meet that on-time-ish frame
// with a stop a few ms old whose jerk (up to the stop limit, 3.5e5 rev/s³ hand)
// has already opened Δa ≈ 1000+ rev/s²: refused, latched, E-STOP — on every
// overrun. Instead the previous frame's span-2 cubic simply CONTINUES (C∞ — it is
// the same curve) for SCHED_GRACE_S; a same-plan frame landing inside the window
// differs only by the plan's own knot jerk change (δ = 20 ms, Δj 2.7e4 → 550
// rev/s², 5.5 rev/s, 0.04 rev) and is promoted as a normal PLAY frame (counted by
// the diagnostics). Exhaustion therefore needs a gap of 2T + GRACE from the last
// frame's stamp. The price is ≤ 20 ms of open-loop extrapolation at the span's
// jerk before the stop starts from the extended state (C2 at the junction).
constexpr float    SCHED_GRACE_S              = 0.020f;
// C2 stop profile limits (hardware_config.yaml trajectory_op.stream_stop_*).
constexpr float    SCHED_STOP_ACCEL_LEG_RPS2  = TrajOp::STREAM_STOP_LEG_ACCEL_RPS2;
constexpr float    SCHED_STOP_JERK_LEG_RPS3   = TrajOp::STREAM_STOP_LEG_JERK_RPS3;
constexpr float    SCHED_STOP_ACCEL_HAND_RPS2 = TrajOp::STREAM_STOP_HAND_ACCEL_RPS2;
constexpr float    SCHED_STOP_JERK_HAND_RPS3  = TrajOp::STREAM_STOP_HAND_JERK_RPS3;
constexpr float    SCHED_STOP_BRACKET_S       = 0.004f;   // first bisection bracket
constexpr float    SCHED_STOP_MAX_S           = 0.5f;     // duration cap (counted if it binds)
constexpr uint8_t  SCHED_STOP_BISECT_ITERS    = 20;
constexpr float    SCHED_STOP_REL_EPS         = 1e-4f;    // float slack on the accel bound
// Hand acceleration-FF gain K from the wire: finite, clamped to [0, this].
constexpr float    HAND_FF_GAIN_MAX           = 1.5f;

// ── FW 22 hand acceleration torque feedforward (U2b; trajectory_op yaml) ─────
// tau = fade * sat(Ks * J * 2pi * a_cmd + bias). J is the MEASURED reflected
// inertia (the larger identification — feedforward sized on it cannot exceed
// the physical J*alpha by the declared-vs-measured gap). Clamp 0.234 N m =
// 0.85 × 50 A × Kt 0.0055133; at scale 1000 that is 234 int16 counts.
constexpr float    HAND_TORQUE_FF_CLAMP_NM        = TrajOp::STREAM_HAND_TORQUE_FF_CLAMP_NM;
constexpr float    HAND_FF_GAIN_SLEW_PER_S        = TrajOp::STREAM_HAND_FF_GAIN_SLEW_PER_S;
constexpr float    HAND_TORQUE_BIAS_CLAMP_NM      = TrajOp::STREAM_HAND_TORQUE_BIAS_CLAMP_NM;
constexpr float    HAND_TORQUE_BIAS_RATE_NM_PER_S = TrajOp::STREAM_HAND_TORQUE_BIAS_RATE_NM_PER_S;
constexpr float    HAND_TORQUE_FADE_PER_S         = TrajOp::STREAM_HAND_TORQUE_FADE_PER_S;
constexpr float    HAND_TORQUE_FF_J_2PI           = HandEnv::MEASURED_REFLECTED_INERTIA_KGM2 * 6.2831853f;   // N m per rev/s^2
constexpr uint8_t  HAND_TORQUE_DRAIN_TICKS        = 3;   // tau=0 re-sends after hand TX stops (6 ms)
...
// the command to the encoder, and once the slider is jammed at the clip value the
// two AGREE (residual ~0, guard silent, motor pushing into metal); and the
// MAX_LEAD_HAND_REV clamp anchors the setpoint TO the encoder (fb + clamped lead),
// so it re-issues the jam rather than refusing it. The margin is the only thing
// standing between a saturated command and the stop.
// Both terms come from hardware_config.yaml jugglebot_geometry
// (hand_motor_hard_stop_revs / hand_clip_margin_rev) — never a literal here.
constexpr float HAND_MOTOR_MAX_POSITION =
    Geometry::HAND_MOTOR_HARD_STOP_REVS - Geometry::HAND_CLIP_MARGIN_REV;   // 10.501
...
constexpr float HAND_TOR_SCALE = InputScale::hand_tor;  // 1000.0 since FW 22 (was 100: 1.81 A/count)
static_assert(HAND_TORQUE_FF_CLAMP_NM > 0.0f && HAND_TORQUE_FF_CLAMP_NM * HAND_TOR_SCALE < 32767.0f,
              "hand torque clamp must fit the int16 wire field");
