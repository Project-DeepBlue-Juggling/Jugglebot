...
- **Jetson GL rendering.** The Jetson Orin Nano's NVIDIA Tegra drivers provide EGL but may not include GLX out of the box. The MuJoCo **viewer** (GLFW-based windowed rendering) requires GLX — `MUJOCO_GL=egl` only works for offscreen/headless rendering, not for the interactive viewer window. If the viewer fails with `GLX: Failed to load GLX`, install GLX support: `sudo apt install libglvnd-dev libgl1-mesa-glx libglx-mesa0`. Verify with `glxinfo | head`. For headless testing (no viewer), `MUJOCO_GL=egl` or `MUJOCO_GL=osmesa` with `libosmesa6-dev` works.
