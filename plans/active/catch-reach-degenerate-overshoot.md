...
### Phase 5 — Seat-experiment prep (the seat rate becomes a config key)

Phase 4 left exactly one question open — **does a non-zero seat rate widen the
capture basin?** — and left it unanswerable, because moving the seat rate meant
editing `planner.py` at the bench. This phase makes the rate a config key and
gives the pre-registered A/B a protocol and an instrument. No commanded-motion
change ships: the generated default is `0.0`, identical to the literal it replaced.

Scope, in three parts:

1. `trajectory_op.catch_seat_rate_radps` in `config/hardware_config.yaml`
   (generated `JB_TRAJ_CATCH_SEAT_RATE_RADPS`), bound **once at import** into the
   existing `planner._CATCH_TILT_THROUGH_RATE_RADPS` so it stays the same plain
   module attribute the 12 existing monkeypatches target.
2. `tests/hardware/session_anomaly_fixes.md` § SECTION SEAT-EXP — a warm-up-first,
   two-block, pre-registered protocol with a mandatory revert.
...
accounted for exactly (6 new probe tests, 3 new drift guards); **xfail unchanged
at 3**.

Two defects were surfaced by review and fixed in-phase rather than deferred:

- The probe's `--self-check` case for the **one bias the design calls
  disqualifying** was vacuous — it grafted its synthetic bounce-out *below* the fit
  plane, where the band filter drops it either way, so a probe with the
  descending-branch cut deleted scored `8/8 PASS, exit 0`. Now grafted back inside
  the band and mutation-verified both ways (mutant `FAIL (1 case)` exit 1;
  unmutated `PASS` exit 0). This mattered because `SEAT-EXP-4` gates the entire
  scoring pass on that command.
- The drift guard `test_planner_seat_rate_is_bound_from_the_generated_constant`
  was vacuous at the shipped value (`0.0 == 0.0` is satisfied by a re-hardcoded
  literal, the very mutation its docstring named). Now value-independent: it
  rebinds the generated constant to a sentinel, reloads the module, and requires
  the attribute to follow.

Three protocol gates were **criteria that fire on correct behaviour** and were
re-derived from measured numbers: the burn-in criterion (`successive change <
10 mm` fires 3 times in 7 on the settled plateau → replaced by a trend slope), the
matched-arms gate (a flat `5 mm` is only 1.1–1.4 standard errors at the expected
sd and `n`, so identically-aimed blocks fail it 17–28 % of the time → now
`max(5 mm, 2·se)`), and the marginal band (a one-sided floor passes both
under- and over-biased aim → now two-sided, `−15 … −5 mm`).

**Deferred to the operator — this phase ships no hardware result.** § SECTION
SEAT-EXP is a separate, later sitting; § THE RUN SHEET is EXECUTED and is not
re-run. The one open prerequisite is **how to bias BB arrival by +25–35 mm in `x`**:
that is Ball-Butler aim, it is deliberately not prescribed here (inventing an
unverified command is how § CHECK LVL-4 and FW-1 broke), and it should be settled
*before* the robot is powered — the protocol's go/no-go depends on it and has an
explicit INCONCLUSIVE-BY-DESIGN exit if it cannot be met.
