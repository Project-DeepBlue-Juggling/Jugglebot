...
| [clapboard-can3-integration.md](clapboard-can3-integration.md) | active | 2026-08-16 | **Phase 0 COMPLETE 2026-08-16; Phases 1-6 not started.** Electronic clapboard (separate repo, `~/Desktop/Github/Electronic-Clapboard`) on the cone bus in place of the Catching Cone — a labelled slate to camera plus a 50 ms sync flash timestamped against the bridge's time-sync master. Implements the five items that repo's `docs/can-integration-handoff.md` assigns to this side; its `docs/protocol.md` §8 is normative and neither side changes it unilaterally. **The job splits two ways by cost:** uplink is nearly free — `on_cone_rx` has no ID filter and `_on_cone_frame` is already an ID dispatcher, so clapboard frames ALREADY reach the Jetson and are discarded; making them useful is Jetson-only, no firmware, no reflash, no `PROTOCOL_VERSION` bump. Downlink is greenfield: `can_cone_send()` has exactly ONE caller (`time_sync_master.cpp:50`), so both the 2 Hz `CLAP_LINK` beacon and the 41-frame slate push need new firmware. Seven phases: 0 test-surface prep (the `ConeFrame` round-trip hole + header-inline ID predicates, chosen over a 1–2 day FlexCAN host shim the codebase has twice declined), 1 uplink (colcon only), 2 **a single FW 15** (honest `cone_health` + `CLAPBOARD_PRESENT` on heartbeat bit 6 + 2 Hz `CLAP_LINK` + `CLAP_SEND` 0x0060 + paced one-frame-per-tick drain — owner chose one flash over rollback granularity 2026-08-16, so there is no FW 16), 3 `SetSlate` action **on the bridge node — the single-owner UDP hazard, not style**, copying `bb/throw`, 4 cross-repo contract tests, 5 bench soak (operator sitting, not agent work), 6 doc sweep. **Named first-bug candidate: `CLAP_LINK` must be DLC 8** — the clapboard silently drops any other length and the panel sits in screensaver forever with no error on either side; only its code enforces this, its `docs/protocol.md` never states it. CAN3 drive-path risk assessed as modest — the logbook discriminator is drive current from node count, not frame rate, and a clapboard is one node — with the halve-the-drain-rate fallback pre-registered against a named soak criterion. **All five owner decisions RESOLVED 2026-08-16** (fallback accepted, 12 V validated as adequate, this repo owns the golden-vector fixture, harness already wired, single flash) — nothing is blocked on a decision |
