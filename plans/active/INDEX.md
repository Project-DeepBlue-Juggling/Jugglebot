...
| [operator-observability.md](operator-observability.md) | active | 2026-08-21 | **Operator observability quartet, on worktree branch `2026-08_operator-observability` (base `5de4a1c`, independent of the ILC session).** Four owner-requested features, all software-only, ordered F3→F4→F1→F2: **F3** ODrive errors decoded by name+axis to the launch shell and GUI event log (the full bitmasks already reach `/robot_state`; the unused `can/odrive.py::error_names` table becomes live again — parity row 62 closes; row 35 closes bar the disarmed-axes list); **F4** `bb/calibrate` refused when QTM is not delivering ≥3 BB fiducials (incl. Marker 3), HOMING skips-not-faults, plus calibration hardening (arc-span floor, mid-sweep invalidation, collection timeout); **F1** charts in physical units (per-leg mm, hand 31.63 mm/rev, BB pitch absolute deg = 90+360·rev) + the `main.js` hand-factor 2.2× bugfix; **F2** the ROS2 Topics panel toggleable to true per-type UDP `msg/s` from `LinkStats` counters via a new 1 Hz `/udp_diag`. Two audit points (after F4, after F2), `--full` at each pair closure. **SOFTWARE-COMPLETE 2026-08-22** — all four features + both audit-fix commits landed and pushed; remaining: § 8 operator bench checklist, then owner-timed merge-back |
