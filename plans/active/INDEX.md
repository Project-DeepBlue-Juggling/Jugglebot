...
| [kinematic-calibration.md](kinematic-calibration.md) | active | 2026-09-23 | **Fit the Stewart geometry to a mocap pose sweep (owner design, 2026-09-23).** The Platform sits ~8.5 mm laterally from its command in the QTM (= Base) frame, growing with height (0.77° lever arm) — model-geometry error in a hand-built FDM machine, not a frame artefact. Remedy is a parametric calibration (48 geometry params + 3 body-registration angles, CAD prior, freeze-what-data-can't-pin), NOT a mocap pose-servo — that returns only if the pre-registered path-dependence test reads RANDOM (> 1 mm within-direction). Two-part: node geometry once, the six leg zero-offsets re-fitted per session (homing repeatability unmeasured). Sweep: reachable set inside ±300 mm x/y, z 100–250, ≤ 30 min. Done = hold-out ≤ 1 mm RMS / ≤ 2 mm max / ≤ 0.1°, then the frame check reads ≤ 1–2 mm at every z and the landing subtraction retires. Prerequisite for learner z authority; does NOT block skill-stack R4. ⚠ Geometry edit waits for R4's commit (shared box re-sweep) |
