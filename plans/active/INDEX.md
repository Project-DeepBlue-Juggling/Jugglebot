...
|------|--------|--------------|-------|
| [cup-contact-contract.md](cup-contact-contract.md) | active | 2026-09-18 | **DESIGN, owner-approved in principle — next unit after the 2026-09-18 sittings.** One root cause behind four symptoms: the banking prescription `tilt_to_receive(g − a_cup)` has no solution in the pre-catch dive (apparent gravity points up), clamps to 12° with a scale-free azimuth (12.000° at every lateral offset from 0.5 to 40 mm, 2026-09-16), the widen loop picks a discrete jerk branch, so the learner's lateral channel is pinned, the tracker aim is clamped, timing re-aims refuse LIMIT_JERK, and the cup outruns an early ball (> g run-up, HELD/EMPTY/HELD). Three clauses: C-CUP-1 banking defined only under seating force (carry the last valid attitude, amplitude-aware demand); C-CUP-2 the cup never accelerates downward faster than κ·g (0.7) inside the contact window (touch-down − 60 ms → release/rest), as a QP row and a `validate_cycle` refusal `CUP_CONTACT_ACC`; C-CUP-3 jerk-aware widen exit. Prerequisite: the mocap base alignment (owner: +30…−50 mm platform offset from a small base misalignment, extra base marker) and a session-start frame check. Unpins `learner_lateral_authority_mm` → 40 and the tracker aim with it. Boxes re-swept (gate_hash changes). On R4's critical path (250 mm sites = lateral aim every throw). |
