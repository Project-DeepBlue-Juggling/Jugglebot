...
instead of maintaining a second list. **Every addition is low-rate, without
exception** — including `/trajectory/commanded_position`, which this section
originally called a 40 Hz `Point` and built its bag-size caveat on. It is not:
it is published inside `trajectory_node._publish_status`, on the
`create_timer(0.2, ...)` **5 Hz** timer (corrected 2026-08-11; the launch-file
comment and `_live_commanded_position`'s docstring both already said 5 Hz). So
the bag-size objection to collapsing the lists does not exist at all. A missing
topic is unrecoverable after the fact; a recorded silent topic costs nothing —
the launch file already argues exactly this for `/leg_lengths_topic`.
...
```
on_empty_cup:  STOP (default) | RELOAD      # reachable under BOTH stop_on_miss settings
...
**2a–2f are BUILT, INDEPENDENTLY VALIDATED and AUDITED** (2026-08-11). The audit
returned NOT CLEAN — eleven findings, **2 HIGH, 6 MEDIUM, 3 LOW** — of which
**seven** landed as fixes in the same session. The three that changed what a row
above claims:

* **2d — the `ABORTED_NO_RELEASE` retry had no settle floor** and started on the
  next FSM tick, inside its own `ACTION_SAFE_ABORT` teardown. Beyond the two
  refusals the MISS floor already prevents, it would have broken the *two
  consecutive non-releases stop the session* gauge, because the retry would have
  died `REJECTED_HAND_NOT_PARKED` instead. Fixed; the floor is now stated as
  belonging to the SAFE_ABORT **ladder**, with all three continuations sharing
  one constant.
* **2d — the IDL's "RELOAD is reachable only with `stop_on_miss` false" was
  false.** The dwell bounce-out route (a CAUGHT cycle, then `REJECTED_NO_BALL`)
  fires the interlude with `stop_on_miss` **true**, and that route is the
  feature's stated primary purpose. Corrected on the wire and in § 3.9's field
  block above.
* **2e — the session trim seeded its prior from a DORMANT map.** Layer 2 not
  depending on layer 1's dormancy is about the trim's own *measurement*; the
  *prior* is a borrowed number and entered at n₀ = 4, which is D3's double-count
  behind the fence. Dormant now contributes no prior and no `k_v`.

**One HIGH is UNFIXED and constrains the first sitting:** a session cancel during
the auto-reload interlude is honoured in any reload phase, including with a BB
ball airborne, and runs `_safe_abort()` under it. The reload FSM has no
`_toss_cancel_deferred` equivalent, so choosing which phases defer is a design
decision on a hardware-validated path, not a finalisation edit. **Until it is
made: do not cancel a `TossContinuous` session during a reload interlude — cancel
in the dwell.** Full write-up, including everything left unfixed:
`logbook/2026-08-10-toss-selftuning-build.md` § Audit fixes.
