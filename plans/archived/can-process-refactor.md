...
Additionally, the CAN node's 0.3 rev step-limit safety check silently rejects commands with **no feedback to the motor guard**. If a command is rejected, the motor guard continues interpolating forward, unaware that the motors haven't moved. This created a positive-feedback cascade during the Phase 3.1 oscillation incident (see `plans/archived/mpc-oscillation-analysis.md`).
