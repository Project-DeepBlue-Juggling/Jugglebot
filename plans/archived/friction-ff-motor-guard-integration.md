---
title: Friction-FF Integration into motor_guard
status: completed
archived: 2026-05-08
---
...
**Predecessor:** [plans/archived/motion-onset-deadtime-investigation.md](motion-onset-deadtime-investigation.md) — bench-validation phase complete
...
| [motor_guard.py:784](../../ros_ws/src/jugglebot/jugglebot/motion/motor_guard.py#L784) | `self._commanded_torque_ff_Nm = self._mpc_base_torque_Nm.copy()` — `_mpc_base_torque_Nm` is the per-tick torque_ff carried via the MPC IPC envelope. On hardware paths today this is **non-zero in operational runs** — confirmed empirically 2026-05-07 via the pre-flight diagnostic (`tools/check_vel_ff_plumbing.py`): `torque_ff` is live in 99.6 % of samples during a slow z=180 mm move, with peak magnitudes 0.013–0.041 Nm per leg. Source of the value: [controller/hardware_plant.py:769-797](../../controller/hardware_plant.py#L769-L797) calls `cartesian_to_motor_commands(...feedforward_enabled=True...)`, which computes **rigid-body dynamics torque** (gravity + platform inertia, projected through the Jacobian — see `ros_ws/.../motion/dynamics.py`). This is orthogonal to friction FF: the dynamics torque models bulk rigid-body acceleration, the friction FF will model sliding friction at the leg's transmission. The friction FF must **add** to `_mpc_base_torque_Nm`, not replace it, so the two stack cleanly. |
...
# per-leg via the diagnose detector following plans/archived/friction-ff-motor-guard-integration.md §4.
friction_ff:
...
1. **Is motor_guard already populating vel_ff in operational runs?** **RESOLVED 2026-05-07: yes, vel_ff is LIVE.**  Pre-flight diagnostic (`tools/check_vel_ff_plumbing.py`) captured 4 555 telemetry samples during a 12 s z=180 mm move and found:
   - `leg_vel` populated in 99.8 % of samples; peak magnitudes 0.79–0.80 rev/s across all six legs (within 0.005 rev/s of each other → uniform sign convention confirmed; matches the user-stated `+ve = extension` convention pre-can_node).
   - `leg_torques` populated in 99.6 % of samples; peak 0.013–0.041 Nm per leg — the existing **rigid-body dynamics FF** (gravity + platform inertia from `cartesian_to_motor_commands` in `motion/dynamics.py`).
   - **PR 1 collapses to just the Kt promotion** (+ a one-paragraph logbook note documenting the diagnostic). Skip ahead to PR 2 for the friction-FF compute + schema.
...
- Original investigation: [plans/archived/motion-onset-deadtime-investigation.md](motion-onset-deadtime-investigation.md) — symptom recap, candidate-mechanism enumeration, fix-option ranking.
