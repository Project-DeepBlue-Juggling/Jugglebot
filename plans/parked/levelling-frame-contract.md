...
created: 2026-07-25
status: parked   # 2026-08-15 — contract C-LEVEL-1/1.O shipped; Phase 4 residue = LVL-2 (~30 s of robot time, bundle with a catch sitting) and one OPEN DEFECT with no other owner: the Platform Teensy truncates the persisted offset to int16 milliradians (the STATE_WRITE int16 encode, Teensy_code_platform.ino:443-444; 0.0264° measured / 0.0573° worst-case per axis against the ±0.05° band, and LVL-3 is structurally blind to it) — address at the next levelling touch. The third residue item, the LG-3 reachability decision, is DISCHARGED 2026-08-16 (logbook/2026-08-16-levelling-complete-ownership-resolved.md): the state IS reachable, Route A = restart trajectory_node alone after `level`, so LG-3 now costs robot time, not paper.
related_logbook:
...
| 4 | Hardware validation (operator-run) | park plateau within ±0.05° of `(−tilt_x, −tilt_y)` via `tools/probes/levelling_tilt_bag_check.py`; first `go_home` worst leg 2.77 ± 0.30 mm; mocap parked tilt within ±0.10° of level | **PARTIALLY VALIDATED 2026-07-27** — sitting run from § THE RUN SHEET; verdicts in `logbook/2026-07-28-anomaly-fixes-validation-sitting.md`. **PASSED**: `LVL-1` (orchestrator→trajectory_node `+1.157 ms`, status flip `+86 ms`); `LVL-3` on **all three** stages (park `rx −0.8021° / ry −0.1146°`, err **+0.0000 / −0.0000** at stage 6; stage 4 needs `--t0 190.0` or the documented `NOTE:` false-FAIL fires; stage 5 err `−0.0000 / +0.0001`); `LVL-4` REPORT-only (world-frame measured step `+0.8227°` vs commanded `−0.8175°`, ratio **1.0064**); `LG-1` (refusal inert — all six legs span `0.0000 rev`, hand `pos_cmd` span `0.00000 rev`, terminated ≤ 0.667 s); `LG-2`; `LG-4` (exactly 1 refusal all sitting, the deliberate one); `LG-5` **reconstructed** across processes (1st outcome → flip `+30.401 s`, flip → 2nd outcome `+149.778 s`); `FW-1`/`FW-2` on all six launches. **NOT VALIDATED**: **`LVL-2` was never run** — no `go_home` was issued anywhere in the six launches, so the return-to-neutral install path is still unobserved post-`C-LEVEL-1` (≈30 s of robot time; bundle with `CCATCH-4`); `LG-3` **INCONCLUSIVE on the sitting, but its reachability is SETTLED 2026-08-16** (`logbook/2026-08-16-levelling-complete-ownership-resolved.md`) — the precondition was never established because the auto-push **won 7/7** of the sitting's `/gravity_offset` publishes (0.43–3.50 ms, on a structural 2.7–4.3 s head start), the **boot-push subset scoring 5/5** independently. The two-analyst standoff is CLOSED, and **both were partly wrong**: the Platform-Teensy power-cycle recipe is **REFUTED** (that cycle zeroes `levelling_complete` too, giving LG-1's state, not this one's), while "no power-cycle of any board reaches it" is **right** but "so the only honest closure is a unit test" is **too strong**. The state is deterministically reachable with no power-cycle, because the orchestrator's gravity push is one-shot per orchestrator boot, `/gravity_offset` is VOLATILE with no re-request path, and `trajectory_node` starts `_gravity_correction_loaded = False`. **Route A** (preferred): `level`, then restart **`trajectory_node` alone** with the orchestrator left up. **Route B** (fallback): the original whole-graph relaunch, which reaches the state only by *losing* the discovery race it won 7/7. **Caveat to carry**: `trajectory_node` is the sole binder of `:5557`, so the standalone restart's re-bind is unexercised — one bench check before trusting Route A. Both routes are written out in the runbook § CHECK LG-3; **LG-3 is now robot time, not a paper decision**; `LG-5`'s own single-trace reader is NOT-SCORABLE (recorder started 3 min 4 s after the flip); `FK-3`'s "to the last digit" arm has no pre-change reference. **NEW DEFECT**: the Platform Teensy **truncates the persisted offset to int16 milliradians** (`Teensy_code_platform.ino:430`, a C cast), costing **0.0264°** this sitting — half the ±0.05° band — and worst-case **0.0573° per axis**, which exceeds it. `LVL-3` is **structurally blind** to this (it scores the commanded park against the same quantised offset it was given). Any capture taken after a relaunch must be scored with the **persisted** `[0.0140, 0.0020]`, not `LVL-1`'s printed `[0.014455, 0.00207]`. **Criteria REVISED TWICE; the runbook is the authority.** Run it from `tests/hardware/session_anomaly_fixes.md` § THE RUN SHEET (rows LVL-1…LVL-4, LG-1…LG-5), not from § Phase 4 below. **First revision** (in-run): "commanded `rx` flat to ±0.05° across the whole goal" is unreachable — replaced by the park-plateau criterion. **Second revision (2026-07-27)**: § Phase 4's "catch error ≈16 mm and pre-throw swing ≈+2.92°, both unchanged, REPORT-only" is now WRONG in both halves — `catch-reach-degenerate-overshoot` Phase 2 (C-CATCH-1) removed the 0.3008° through-seat residual that caused the 16 mm **and** the arrival twist that caused the swing. Catch error is now a GATED `< 10 mm` (runbook CCATCH-2t, a prediction not yet measured) and the reach is expected FLAT |
