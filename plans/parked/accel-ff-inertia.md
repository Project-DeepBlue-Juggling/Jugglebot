...
- The Phase-2-validated torque predictor is also the **frozen model baseline** over which `plans/parked/learned-ff-residuals.md` (batch ILC) learns torque-trim profiles. Conclude this plan (shipped enabled OR dormant — either freezes the baseline) before ILC learning starts (its gate G-B); learned profiles key on the model-config hash, so any sizing/flag change here auto-invalidates them rather than silently mis-applying.
