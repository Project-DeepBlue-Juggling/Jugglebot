...
| `juggle_motion_quality.py` | Motion-quality probe (Fable-5 review P0, 2026-07-03): instruments the cup (`hand_opening` site) vs the true ball each control tick and reports cup vertical path length/cycle, slider stroke span + ceiling/floor clamp-saturation ticks, disengaged-travel fraction (cup moving while not interacting with the ball), max per-tick z command step, and **cup vz at first ball contact (sign + ratio)** — the "receive" check (a smooth catch contacts with the cup moving DOWN). Finding: the self-catch MAKE cycle uses **1026 mm/cycle**, slams the stroke ceiling **93 ticks**, is **45% disengaged**, has a **60 mm/tick command step**, and receives the ball on a **RISING** cup (+10 mm/s) — vs the Rung-1 catch's 135 mm/cycle, no clamps, receives down. Grounds the P0 gate + the P2 re-unification. Headless MuJoCo, no hardware. Writes `temp/probes/juggle_motion_quality.csv`. | `logbook/2026-07-03-motion-quality-review.md` |
