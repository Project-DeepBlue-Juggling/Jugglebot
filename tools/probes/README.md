...
| Probe | Purpose | Motivating logbook entry |
|-------|---------|--------------------------|
| `profile_session.py` | Records `tegrastats` (CPU/EMC/thermal/power), `pidstat` (per-process CPU / context switches / IO wait), `mpstat` (per-core utilisation), optional `py-spy` flame graph, plus a pre-flight snapshot (nvpmodel / governors / max freqs). Auto-analyses the recording into a `report.md` that flags CPU saturation, DVFS throttling, thermal warnings, EMC saturation, and memory pressure. Subcommands: `start` / `stop` / `record` (Ctrl-C-able blocking mode) / `analyze`. | _logbook entry pending_ — built during the Jetson-load-profiling investigation that followed the warm-start / overhead-spikes work in `logbook/2026-04-18-overhead-spikes.md` and `logbook/2026-05-20-walk-forward-singleton-emission-jerk.md` |
