...
| `juggle_selfcatch_loopgain.py` | Rung-2b single-ball self-catch loop-gain sweep (the MAKE-OR-BREAK gate evidence): drives `sim/juggle_selfcatch.py::run_self_catch` (compose the Rung-2a tilt-aimed throw with the Rung-1 catch into a single-ball toss→catch→toss loop, re-planned each cycle under §3 tracking noise) across seeds × three recover/geometry variants (faithful plan_cup_cycle + stationary column, drift in-place, ad-hoc axial retract), reporting the per-cycle **reach trend** and the **sustained cycle count**. **Finding: the pure column self-catch does NOT sustain (loop gain > 1)** — every seed and variant diverges within 0–3 cycles (the catch reach amplifies ~8 → ~50 → ~110 → ~210 mm past the catch's ~60–80 mm reliable reach → drop). Tilt is ~0 for a column, so the tilt mechanism never engages. Grounds `tests/sim/test_juggle_selfcatch.py`. Headless MuJoCo plant, no hardware. Writes `temp/probes/juggle_selfcatch_loopgain.csv`. | `logbook/2026-07-01-rung2b-selfcatch-column-divergence.md` (Phase 3 / Rung 2b of `plans/active/bb-online-juggle-tilt-rearchitecture.md`) |
