...
- **Current arc (2026-09-09): `plans/active/two-ball-skill-stack.md`, branch `skill-stack`, worktree `~/Desktop/Jugglebot-skills`.** A schedule-driven throw/catch skill stack after Lee et al. (RAI, arXiv:2608.26800): skills dispatched on the shared CAN wall clock, every segment rest-terminal, one hand master (the can-bridge streamed lane; the Platform Teensy stroke engine retires at R1), and a memory-based learner whose command is the commanded landing (identity prior). **Its § 0 values are normative for all work on that branch.** The unified 7-DoF ring, the FSM toss/reload choreography and the critical-point ILC are SUPERSEDED (archived 2026-09-09 with notes naming what survives) — do not extend them; the FSM stack is deleted at R4 under tag `fsm-final`.
...
- **`controller/` is the shared motion-primitive layer**, not an MPC package: `ballistics`, `feasibility` (K1–K6), `hermite`, `plant`, `target`, `telemetry`. Live sim and hardware paths import from it; it has no CasADi dependency since the MPC removal. `scheduler`, `catch_optimizer`, `toss_motion_source` and `zmq_target` were deleted 2026-09-09 (R0 dead-layer deletion, `plans/active/two-ball-skill-stack.md` § 6) — MPC-era sim sources with no non-test importer left after the MPC chain removal. See `controller/README.md`.
...
- **Token budget governs agent use (owner, 2026-09-07 — measured, not a preference)**: the owner is limit-bound. Across 2026-08-30..09-07, 162 agents consumed ~1.87 billion cache-read tokens against ~229 million on the main threads, and the 14 longest agents (150–390 calls each, peak context 400–700k) were **half of all agent cost**, because an agent's cost is *calls × accumulated context*. Three hard rules:
  1. **Cap agent LENGTH, not just count.** Scope every `Agent` to a unit it finishes in **under ~80 tool calls**. A bigger unit is split: the agent writes a handoff file in the scratchpad (state, done, next, gate result) and stops; a fresh agent continues from the file. Never hand one agent a whole wave or phase — the Phase 4 Wave B agent ran 387 calls at up to 619k context for 160 M cache reads; four ~100-call agents would have done it for roughly a third.
  2. **Tier the models; Sonnet is the default.** `model: sonnet` for implementing a specified design, tests, docs, audits and fix application; `model: haiku` for mechanical edits; **`model: opus` only for design-bearing or safety-bearing units** (planner math, firmware, hardware diagnosis), at most one or two per unit, each justified in one line of the brief. The main session makes small edits itself — an `Edit` costs less than any agent. This narrows the 2026-08-15 orchestration-only agreement in memory: the *role split* stands, its *Opus-everything default* does not.
  3. **Brief with excerpts, never the whole plan.** Extract the phase section plus only the files the unit touches into a scratchpad file and point the agent at that. A plan past **~400 lines** has become a changelog: its phase narrative belongs in the logbook entry that already records it, and the plan keeps decisions, interfaces and status (`unified-7dof-planner.md` reached 1,684 lines and was the largest single tool result in four of six sessions).
  Two cost rules follow from the same numbers. **One review pass per phase**: a single `/audit --unstaged` at phase end whose fixer applies narrative/comment/test-metadata findings directly and surfaces only behaviour-affecting findings for adjudication — no lens → fix → audit → fix chains (Phase 4 ran seven review-shaped agents, and its first audit's 12 findings were all narrative). **Compact at phase boundaries**: right after each commit + push, `/compact` or hand off to a fresh session — the main thread ran at 300–900k tokens per call this arc because compaction only fired at 903k. Console output reaches Claude **by path** (runbooks `tee` to `temp/logs/`, Claude reads with `grep`/`sed`), never by paste — 62 % of the owner's turn volume was pasted logs that then rode every later call.
...
- **Dress-rehearse every rung before the robot is powered**: with the launch running on the *loaded* Jetson (bag recording, GUI up — the load the solve will actually see), run each rung's dry-run / validate-only path against the LIVE `/robot_state`, and make gates report **every** refusal at once rather than stopping at the first. UH-3 (2026-09-06) was refused three deep and then E-STOPPED across three sittings for one physical fact the machine knew and the planner did not; its Defect 2 was "found offline; the operator never saw it", the sitting's own seed replayed all three, and the 135× solve inflation only appears under load (the idle probe returned a null). A rung that has not passed its rehearsal is not on the runsheet.
- **Session start: claim the nightly ticker, read the status ONLY if you won it**: `tools/nightly_ticker.sh check --who "<what this session is for>"` — one line, and it is the whole check. `CLAIMED <status>` means you are the first session since the 04:00 run: read `temp/reports/nightly/status` / `latest.md` and surface the result in your first substantive reply. `LOWERED …` means another session already handled today's run: **print that one line and read nothing else** — do not open `status` or `latest.md` (owner ask, 2026-09-08: with several sessions a day every one was re-reading and re-diagnosing the same RED, often one session's own 04:00 work-in-progress, since the nightly runs against the working tree). `STALE`, `UNRAISED` and `NEVER` are the runner-health cases below. Subagents never run the check. The status line is `GREEN|RED|DEFERRED <counts> <iso-date>` and is the ONLY delivery channel for the 04:00 full-tier run (owner's choice: no email, no GUI), so an unread RED is an invisible RED. **If you claimed it, anything that is not a fresh GREEN gets surfaced**, before other work: RED → list the failures from `temp/reports/nightly/latest.md`; DEFERRED → a live robot session held the box, so no run happened; a date more than ~2 days old → the runner itself stopped; **file missing entirely → the runner was never armed on this box, or `temp/` (gitignored) was cleaned** — do not read absence as "fine". For the last three (DEFERRED is a normal CLAIMED status; the stale-date and missing-file cases surface as the ticker's `STALE` / `NEVER`; `UNRAISED` = a status with no ticker, read it once), check `systemctl --user list-timers jugglebot-nightly.timer` and re-arm from `tools/systemd/README.md`. **The nightly runs against the WORKING TREE, not HEAD** — a live session's uncommitted work paints it RED (2026-09-05 and 2026-09-07 both were). Read a RED against `git status` and `git stash list` before calling it a regression; the durable fix is `tools/nightly_suite.sh` running in a detached `git worktree` of HEAD (open follow-up, 2026-09-08).
...
- **Proactively suggest session boundaries**: end each working session as a unit — logbook entry closed, suite green, the logical unit committed — and don't let edits or red tests accumulate across sessions. *Offer* to close out and start fresh at natural boundaries (unit done + committed + logged, context getting long, topic about to switch subsystems); keep it a one-line offer the user can decline. Canonical cost of skipping: the BallButler chapter accumulated 43 uncommitted files and 68 failing tests across four sessions, all surfacing at commit time — each one-file-cheap to fix when introduced. **End every working day with a commit**: a session left running overnight on an uncommitted tree is what the nightly measures and what a parallel session trips over.
