...
# loadfile) then a serial phase for the `serial`-marked allocation/timing
# tests.  ~8 min vs ~28 min for a bare serial `pytest tests/`.
./run_tests.sh

# By category (bare pytest is still right for iterating on one file)
...
# Nightly hypothesis run (max_examples=1000; ci-fast=50 is the per-PR default)
./run_tests.sh --hypothesis-profile=ci-deep
...
- **Run the full suite after code changes AND before `git commit`, automatically**: run `./run_tests.sh` after any change to `*.py` or `*.yaml`, and run it again immediately before the commit gets written — both as part of the same response, without waiting for the user to ask. It runs the suite in a parallel phase plus a serial phase for `serial`-marked tests (~8 min; a bare serial `pytest tests/ -q` is ~28 min and remains equivalent-but-slower if you need it). Scoped subsets (e.g. `pytest tests/motion/`) are fine for iteration but the full suite is the final pre-commit gate. For `*.yaml` changes under `config/` the order is: edit YAML → `python config/generate_config.py` → stage the regenerated artifacts → run tests → commit. Never commit known-failing code without the user's explicit acknowledgement; report the count and result in the same response as the commit. (Use `-q` consistently for the pre-commit gate; `-v` is fine for debugging when a test fails.) **A "docs-only, so no tests needed" exemption is NOT automatically safe, and the justification must name the tests that read the path you changed** — never a bare appeal to the file extension, and never a mechanism you have not traced. Worked example, because getting it wrong twice is how the rule earned its clause: `tests/sim/test_logbook_search.py` *does* parse the real `logbook/` directory (`pytest tests/sim/test_logbook_search.py -q`, run 2026-07-27: **24 passed in 0.19 s**), so a logbook edit is not automatically outside the test surface — **but it would not have caught the two failures you would most expect it to.** `sim/analysis/logbook_search.py` skips `INDEX.md` outright (`_SKIP_FILES`), and `load_entries` silently `continue`s past any entry whose front matter lacks a `title`, while the test asserts only `len(entries) >= 2` plus the shape of the alphabetically-first entry (a 2026-03-30 file). So a broken INDEX row or a malformed new entry passes green. Trace the coverage; do not infer it from a passing count.
...
- **Checkpoint before sinking effort, not after**: domain and physical context lives with the user. Before spending real effort, do a *brief inline sync* — one or two lines stating the approach and the key assumption it rests on — and wait for a quick confirm. Three triggers: (a) before committing to a non-trivial approach, (b) when leaning on a physical/system assumption the user could correct, (c) when deviating from the agreed plan. This complements "invite physical-intuition pushback" above: that rule makes it cheap for the user to *object*; this one makes you *proactively surface* the decision before it is expensive to unwind. For root-cause hunts, **pre-register the fallback and the decision criterion that flips you to it** ("if the kinematic scales aren't stable across two runs, ship the measured offset") so the pivot is planned, not a sunk-cost spiral. Canonical cost of skipping: the 2026-06-10→18 BB temporal-accuracy chapter burned multiple sessions because effort was sunk before a sync — a black-box affine built and deployed before confirming the plan, a "ball-inertia" feedforward coded and flashed that made the lag *worse* (44→56 ms), and a kinematic system-ID pursued for sessions before its instability was accepted (BallButler `logbook/2026-06-18-temporal-accuracy-resolved-fractured-solution.md`).
- **Proactively suggest session boundaries**: end each working session as a unit — logbook entry closed, suite green (the pytest-before-commit rule above), the logical unit committed — and don't let edits or red tests accumulate across sessions. *Offer* to close out and start a fresh session at natural boundaries (a unit is done + committed + logged, context is getting long, or the topic is about to switch subsystems) rather than waiting to be asked; keep it a one-line offer the user can decline. Canonical cost of skipping: the BB chapter accumulated 43 uncommitted files and 68 failing tests across four sessions, all surfacing only at commit time — each would have been one-file-cheap to fix when introduced.
...
- **New tests are parallel by default**: `./run_tests.sh` runs the suite under xdist with `--dist loadfile`, so a file's tests always share one worker. Write tests that hold under that: bind **ephemeral** ports (`:0`, then read back the assigned port — see `tests/teensy_link/conftest.py` and `tests/sim/_zmq_test_harness.py`) and write files via `tmp_path`, never to a fixed shared path. Reach for `@pytest.mark.serial` **only** when the test measures a process-global resource — `tracemalloc` heap growth, GC events, wall-clock timing — whose baseline is corrupted by concurrent load or by unrelated tests sharing the interpreter. Needing a port or a temp file is not a qualifying reason. Marking a test `serial` costs the whole suite serial time, so keep the marked set small (currently 5 tests, ~25 s).
