...
class CANInterface:
    ...
    def _handle_arbitrary_parameter_read(self, axis_id, data, param_type='f'):
        ...
        try:
            ...
            elif endpoint_id == self.ARBITRARY_PARAMETER_IDS['get_gpio_states']:
                # Re-unpack the parameter value as an unsigned 32-bit integer
                _, _, _, states = struct.unpack_from('<BHBI', data)

                # We only care about GPIO 3
                gpio_3_state = (states >> 3) & 0x01  # Extract bit 3

                # Invert the state so that 1 means pressed (connected to ground)
                gpio_3_state = not gpio_3_state

                # Log the GPIO states
                self.ROS_logger.info(f"GPIO states read on axis {axis_id}: {gpio_3_state}")
