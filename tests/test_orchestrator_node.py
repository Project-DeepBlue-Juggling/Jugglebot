...
class TestOnCommand:
    def test_queues_command(self, orch):
        ...
        assert orch.ctx.pending_command == 'activate'

    def test_overwrites_unconsumed_command(self, orch):
        orch.ctx.pending_command = 'old'
        msg = MockString(data='new')
        orch._on_command(msg)
        assert orch.ctx.pending_command == 'new'
    ...
    def test_various_commands(self, orch):
        for cmd in ['activate', 'deactivate', 'spacemouse', 'shell', 'home', 'clear_errors']:
            ...
            orch._on_command(msg)
            assert orch.ctx.pending_command == cmd
...
class TestTick:
    ...
    def test_control_mode_not_published_when_same(self, orch):
        orch._tick()
        count_after_first = len(orch._control_mode_pub.published)
        orch._tick()  # Same mode, should not republish
        assert len(orch._control_mode_pub.published) == count_after_first
    ...
    def test_state_not_published_when_same(self, orch):
        orch._tick()
        count = len(orch._state_pub.published)
        orch._tick()
        assert len(orch._state_pub.published) == count
...
class TestTickIntegration:
    def test_boot_to_homing_via_tick(self, orch):
        """Heartbeats arrive → state machine advances from BOOT to HOMING."""
        orch._tick()  # Enter BOOT
        assert orch.sm.state == RobotState.BOOT
    ...
    def test_command_queued_and_consumed(self, orch):
        ...
        assert orch.ctx.pending_command is None
