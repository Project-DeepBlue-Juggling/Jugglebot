...
| **H7.4** | `dip_below_x3` | timeline probe, row 4 | `<= 0.100` rev, unchanged | `> 0.100` rev ⇒ **the correction over-braked.** This is the one failure mode C-HAND-2's "declared inertia ≤ measured" clause exists to prevent, so it is a *contract* violation, not a tuning miss: lower `teensy_trajectory.throw_decel_reflected_inertia_kgm2`, do not raise it. Record the value and the tier |
...
| CHECKING gates | tier, lead, flight band, event_vel, workspace | **plus** a known throw site, `\|B − A\| ≤ toss_max_displacement_mm`, the closed-form reach bound, and a tilt clamp — all **before** the workspace box |

Direction of risk, stated honestly because it cuts both ways. The **gate** got
...
> **AMENDED 2026-07-29 — read § SECTION DISP before running this section.** Two of
> this section's premises moved. (1) The cap is now
> `jugglebot_operational.toss_max_displacement_mm` = **150 mm**, not 70. (2) The
...
| | before | after |
|---|---|---|
| terminal instant | landing + **0.70 s** (`catch_confirm_window_s`; **0.80 s** since 2026-08-21, when it became derived from `ball_possession.ARRIVAL_BAND_MAX_S` — the row below is the 2026-07-28 measurement and is left at the value it was taken against) | the CAUGHT tick — measured landing + **0.202 – 0.442 s**, median **0.209 s**, over the 17 self-tosses |
...
| displacement cap | `70 mm`, hard-coded | `jugglebot_operational.toss_max_displacement_mm` = **`150 mm`**, operator-adjustable |
...
```bash
INST=~/Desktop/Jugglebot/ros_ws/install/jugglebot/lib/python3.8/site-packages/jugglebot
grep -n "JB_OP_TOSS_MAX_DISPLACEMENT_MM\|JB_OP_TOSS_STAY_AT_POSE_ON_CAUGHT" $INST/hardware_config.py
...
| # | quantity | PASS | ABORT |
|---|---|---|---|
| DISP-0.1 | the two new constants | `JB_OP_TOSS_MAX_DISPLACEMENT_MM = 150.0` and `JB_OP_TOSS_STAY_AT_POSE_ON_CAUGHT = True` | missing, or a different cap ⇒ stale install, or an operator lowered the cap deliberately (fine — then **read the cap off this line and use it everywhere below**, do not use 150) |
...
> **Why `(140, −140)` and not `(150, −150)`.** `150.0` is the *exact* value of
> `TOSS_XY_LIMIT_MM`, and Phase E added a new pre-check that refuses a Tier-8b
> goal whose **throw site** `A` is outside that box (`abs(ax) > 150.0`). If you
> drive there by hand the commanded pose is an arbitrary float — `(150.4, −149.7)`
> echoes as "~(150, −150)" and refuses `REJECTED_WORKSPACE`. `(140, −140)` keeps
> the whole demonstrative value with 10 mm of margin.
...
| # | quantity | PASS | ABORT |
|---|---|---|---|
| DISP-2.1 | node outcome line | not a `REJECTED_*` | `REJECTED_DISPLACEMENT` ⇒ the throw site is not being read live — **stop**, this is the phase's headline defect. `REJECTED_WORKSPACE` ⇒ the platform is parked outside the `±150 mm` planning box (check `/trajectory/commanded_position` — **both** components must be `≤ 150.0`); drive back inside and retry. That is a correct refusal, not a defect |
...
| # | quantity | PASS | ABORT |
|---|---|---|---|
| DISP-4.1 | goal accepted | yes | `REJECTED_DISPLACEMENT` ⇒ the platform was not at centre, or the cap was lowered (DISP-0.1) |
...
#### DISP-5 — displaced **150 mm** (the cap; the operator's ordered working point)
...
```bash
# REPEAT THIS PAIR for each of the 5 attempts. The go_home is MANDATORY between
# attempts, not decorative: a caught toss at the cap parks the platform OUTSIDE
# the +-150 planning box, so attempt 2 refuses without it (see the KNOWN
# LIMITATION box below).
ros2 service call /trajectory/go_home std_srvs/srv/Trigger
ros2 action send_goal /jugglebot/toss jugglebot_interfaces/action/Toss \
...
| CONT-1.8 | `per_cycle_dwell_s` | — | entry 1 is `nan`; entries 2–3 are `≥ 8.0` | any entry **below** `8.0` ⇒ the same fault CS-6 catches, **STOP**. Values above `8.0` are EXPECTED here and are not a fault: a MISSED cycle the session continues past waits out a **2.90 s cleanup floor** from its scheduled landing (`CATCH_CONFIRM_WINDOW_S 0.8` + the `2.0 s` `go_home` profile + 2 node ticks), because `_safe_abort` dispatches the retract and the recentre on service ACKS and returns while both are still moving. At the `5.0 s` delay that floor puts the earliest release at `7.90 s`, i.e. **below** the `8.0 s` request, so the floor should NOT bind here and the entries should read close to `8.0`. **REPORT** anything above `8.60` with the value. (Both numbers moved 2026-08-21: `CATCH_CONFIRM_WINDOW_S` is now DERIVED from `ball_possession.ARRIVAL_BAND_MAX_S` — the measured +798 ms arrival-band ceiling, rounded up — rather than hand-written at 0.7, which sat 98 ms UNDER the band a sensor-primary verdict has to outlast. The pending post-FW14 band re-measure will move both again, from that one constant.) |
...
| `REJECTED_THROW_DELAY` | `throw_delay_s` below the toss FSM's own delay gates, mirrored at session scope so the verdict names the field that is wrong. **The flat `MIN_TOSS_THROW_DELAY_S = 3.5 s` retired 2026-08-22** (census A1); the gates are now `max(TOSS_DISPATCH_DEBOUNCE_S = 0.10, hand_stroke.min_throw_event_delay_s(v_throw))` — the Teensy's own `:642` dispatch budget, **0.337 s** at the 0.4949 s band floor and **0.281 s** at the 0.80 s nominal. Without the mirror a goal with an illegal delay satisfies `dwell ≥ delay + margin`, is ACCEPTED, builds a whole cycle's per-goal state, and only then dies `ABORTED_CYCLE_REJECTED_CANT_MAKE_LEAD` | raise `throw_delay_s` above the floor for **your flight time** (or omit it for the `5.0 s` default). Nothing moved |
| `REJECTED_DWELL` | the dwell is below `max(throw_delay_s + 0.137, hand_floor_dwell_s)`. **Derived, not chosen**, and since 2026-08-22 it has TWO terms (census A3/A4). *Handoff:* a cycle's release is its own accept + `throw_delay`, and the session cannot start cycle N+1 before cycle N's possession verdict can EXIST — `ball_possession.ARRIVAL_BAND_MIN_S` = `+137 ms`, the earliest sensor `empty→held` edge ever observed (it was `0.6 s`, sized on the *mocap tracker's* verdict, until possession went sensor-PRIMARY). *Hand geometry:* catch tail + prelude + gap + throw windup, in series, because C-HAND-1 forbids overlapping them — **`0.4871 s` at the 0.4949 s band floor**, and below ~0.5 s it is the ONLY term that binds. Floor is `5.137 s` at the `5.0 s` default delay | raise `dwell_time_s`, or lower `throw_delay_s` toward its own floor. Do **not** treat the refusal as a defect: a cadence the machine quietly ignored would be a lie about what it did — and a dwell under the *hand* term does not merely run slow, it dispatches the next throw inside the live catch stroke |
