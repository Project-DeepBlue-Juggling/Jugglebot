...
Phase 3 Stage C tests (platform free-standing, unsupported):
  C1. Stable hold at active pose (with torque_ff)
...
# TestODriveErrorTablePins — the copy must stay equivalent to the authoritative
# table.  Before that pin reached this file the copy had drifted: it stopped at
# 0x1000000 and mislabelled that bit (WATCHDOG_TIMER_EXPIRED) as
...
def decode_axis_errors(active: int, disarm: int) -> str:
    ...
    that already bit this robot.  The raw hex pair rides along so a pasted log
    line stays decodable even against a table that later gains a bit.
    """
...
class PlatformTestHarness:
    ...
    def clear_all_errors(self):
        ...
        # very test that just cleared it.  If the ODrives are not broadcasting
        ...
        while time.time() < deadline:
            ...
            for axis_id in LEG_AXES:
                ...
                if self.states[axis_id].has_fault:
                    raise RuntimeError(
                        f"Axis {axis_id} fault during trajectory: "
...
# ===========================================================================
# Test C1: Stable hold at active pose
# ===========================================================================

def test_stable_hold(harness: PlatformTestHarness):
    """Stage C1: Stable hold at active pose with torque_ff.
    ...
    Procedure:
      1. Enter POSITION/PASSTHROUGH, hold current (active) position
      2. Enable gravity torque_ff at active pose
    ...
    """
    print("\n" + "=" * 60)
    print("STAGE C1: Stable hold at active pose (with torque_ff)")
    print("=" * 60)
    ...
    torques_Nm = compute_torque_ff_for_pose(pos_active, rot_active, geom, params)
    print(f"\n  Gravity torque_ff at active pose:")
    ...
    # Move to active pose using TRAP_TRAJ (smooth ramp from current position)
    active_raw = pose_to_raw_positions(pos_active, rot_active, geom)
    print(f"\n  Moving to active pose using TRAP_TRAJ...")
    harness.enter_trap_traj_mode_all(vel_limit=1.5, acc_limit=5.0,
                                     dec_limit=5.0)
    for axis_id in LEG_AXES:
        harness.send(encode_set_input_pos(axis_id, active_raw[axis_id]))
    ...
    # Apply feedforward at active pose
    ...
    for i in range(n_samples):
        ...
        for axis_id in LEG_AXES:
            ...
            harness.send_no_delay(encode_set_input_pos(
                axis_id, active_raw[axis_id], vel_ff=0, torque_ff=ff_int))
...
def test_ff_toggle(harness: PlatformTestHarness):
    ...
    Procedure:
      1. Hold at active pose WITH torque_ff (3s baseline)
    ...
    # Use IK-computed active position as the hold target
    pos_active = np.zeros(3)
    rot_active = np.eye(3)
    hold_raw = pose_to_raw_positions(pos_active, rot_active, geom)
        ...
        for _ in range(n):
            ...
            for axis_id in LEG_AXES:
                ...
                if harness.states[axis_id].has_fault:
                    raise RuntimeError(
                        f"Axis {axis_id} fault during {label}: "
...
def test_step_response(harness: PlatformTestHarness):
    ...
    # Use IK-computed active position
    active_raw = pose_to_raw_positions(np.zeros(3), np.eye(3), geom)
    ...
    active_torques = compute_torque_ff_for_pose(np.zeros(3), np.eye(3),
                                                geom, params)
    for axis_id in LEG_AXES:
        ff_int = int(round(active_torques[axis_id] * LEG_TOR_FF_SCALE))
        ff_int = max(-32767, min(32767, ff_int))
        harness.send(encode_set_input_pos(
            axis_id, active_raw[axis_id], vel_ff=0, torque_ff=ff_int))
    ...
    for axis_id in LEG_AXES:
        actual_raw = harness.states[axis_id].pos_rev_raw
        error_mm = abs(actual_raw - active_raw[axis_id]) / MM_TO_REV[axis_id]
...
def test_gravity_rotation(harness: PlatformTestHarness):
    ...
    Procedure:
      1. Compute gravity torque_ff at active pose and several tilted poses
...
TESTS = {
    'hold':        ('Stable hold at active pose (C1)',
                    test_stable_hold),
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Tests available:
  hold         Stage C1: Stable hold at active pose (with torque_ff, operator release)
