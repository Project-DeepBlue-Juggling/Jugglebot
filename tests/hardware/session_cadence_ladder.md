...
|---|---|---|---|---|
| `catch_position` | `geometry_msgs/Point`, mm, **STOW-relative platform frame** (`z: 170.0` is the ACTIVE plane) | `(0, 0, 0)` — a real request, **not** a default. `z: 0` is 170 mm below the active plane and dies. Always state it | finite; `abs(z - 170)` within `TOSS_Z_BAND_MM` (50.0). **No lateral BOX since 2026-08-29** — the `toss_workspace_xy_mm` knob was deleted; `abs(x), abs(y)` are bounded pre-throw by the closed-form reach bound on `|B − A|` and, on Tier 8b, by the build-time reach-feasibility plan of the deferred A→B catch (`motion/trajectory/catch_reach`); on Tier 8a by `go_to_pose`'s own feasibility gate at the positioning move | non-finite ⇒ `REJECTED_BAD_GOAL(catch_position.x)` / `.y` / `.z`; outside the z band ⇒ `REJECTED_WORKSPACE`; too far to reach ⇒ `REJECTED_DISPLACEMENT` (8b, pre-throw) or `REJECTED_POSITION(<planner code>: …)` — pre-throw at cycle build on 8b, at the positioning move on 8a |
