...
| 4 | **Clean build** (the `extra_script.py` incremental hazard from the 4→5 bump applies to 5→6 identically): `cd ros_ws/src/jugglebot/Teensy_code_canbridge && rm -rf .pio/build && pio run -e teensy41`. Verify the hex md5 against the build-evidence table in `logbook/2026-09-02-unified-7dof-planner-phase3-fw17-hand-lane.md` (FW 17 post-audit-fix build: `2e90c43d5b6889e1059982d2e7aaf961`, 769 144 B hex — reproducible across two clean builds 2026-09-03; supersedes `7abe226b…` after the audit's echo-stamp age-correction). A different md5 means the tree moved: stop and diff. | |
| 5 | **Operator flashes**: close any `pio device monitor` first, then `pio run -e teensy41 -t upload`. | |
