...
REJECT_WIRE_MAP = {
    ...
    'REJECTED_BEAT_TOO_SHORT': 'UH-7a, UNIFIED ONLY, refused at ACCEPTANCE: '
                               'the dwell is under the chain floor, itself the '
                               'larger of (a) the carry `dwell - dt` >= '
                               ...
                               'tick — 0.800 s, and (b) is what binds. A PLUMBING '
                               'floor, not the planner\'s: whether the resulting '
                               'beat is FLYABLE is answered per cycle by the QP, '
                               'and a refusal there is safe because the chain '
                               'holds the machine. Until 2026-09-09 this code '
                               'had a second gate (cycle-1 throw_delay_s under '
                               '0.866 s, a solve-lateness bound); cycle 1 now '
                               'adopts the plan\'s release once the LAUNCH '
                               'answers, so that gate is gone and a short '
                               'throw_delay_s is the session\'s own '
                               'REJECTED_THROW_DELAY (dispatch budget at this '
                               'speed + 0.160 s preamble, ~0.46 s at flight '
                               '0.64). The legacy `dwell >= throw_delay + '
                               'handoff` floor still applies on top but no '
                               'longer binds at that delay, so 0.800 IS a '
                               'unified session\'s real minimum dwell',
