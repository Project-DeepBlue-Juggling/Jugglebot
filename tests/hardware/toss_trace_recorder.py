...
REJECT_WIRE_MAP = {
    ...
    'REJECTED_CANT_MAKE_LEAD': 'goal: throw_delay_s below the DERIVED floor for '
                               'THIS release speed AND this cycle\'s sequence — '
                               'toss_sequencer.min_throw_delay_for_release_s = '
                               'the kind-0 dispatch budget '
                               '(hand_stroke.min_throw_event_delay_s = '
                               'prelude(park band) + SAFETY_GAP + windup; '
                               '0.337 s at the 0.4949 s band floor, 0.281 s at '
                               'the 0.80 s nominal) PLUS the pre-dispatch '
                               'sequence the runtime guard measures it after '
                               '(0.080 s when POSITIONING takes the census-B1 '
                               'skip, 0.460 s when it commands the move) — or '
                               'below the 0.10 s goal-storm debounce, which also '
                               'catches a sign typo. The flat 3.5 s floor retired '
                               '2026-08-22 (census A1); the sequence term landed '
                               '2026-08-23, and without it a goal could be '
                               'ACCEPTED and then abort ABORTED_CANT_MAKE_RELEASE '
                               'every cycle. The message names every term and '
                               'which sequence it charged',
    ...
    'REJECTED_DISPLACEMENT': 'goal (Tier 8b only): |B - A| from the LIVE throw '
                             'site exceeds the closed-form reach bound for this '
                             'flight time (256 mm at T=0.80 s, 108 mm at '
                             'T=0.60 s, 83 mm at T=0.55 s — so a 150 mm goal '
                             'needs T >= ~0.67 s). The bound FOLLOWS the live '
                             'trajectory/set_limits session limits, so a ramp is '
                             'the lever that moves it; the refusal names which '
                             'of vel/acc/jerk bound and quotes the limits it '
                             'judged against. A flat toss_max_displacement_mm '
                             'cap also lived under this code until 2026-08-29 — '
                             'if a trace quotes a "cap", it predates that. '
    ...
    'REJECTED_WORKSPACE': 'goal: catch pose z outside 170+-50 mm. Z ONLY since '
                          '2026-08-29 — the lateral |x|,|y| planning box (and '
                          'its Tier-8b twin on the live throw site A) was '
                          'deleted with its config key, so a far-LATERAL goal is '
                          'now admitted here and refused as REJECTED_POSITION '
                          'instead: PRE-THROW at cycle build by the '
                          'reach-feasibility gate on Tier 8b (which never '
                          'commands B — it pre-positions at the throw site A and '
                          'defers the A->B reach), and at the POSITIONING move '
                          'by go_to_pose on Tier 8a. A trace quoting |B.x| or a '
                          'live throw site under this code predates that change',
    ...
    'REJECTED_POSITION': 'TWO PRODUCERS since 2026-08-29, told apart by PHASE. '
                         '(a) CHECKING, Tier 8b only: the DEFERRED A->B catch '
                         'reach was planned at cycle build '
                         '(motion/trajectory/catch_reach) and refused, so the '
                         'subcode is the PLANNER code (WORKSPACE / UNREACHABLE '
                         '/ LIMIT_VEL / LIMIT_ACC / LIMIT_JERK / STEP_BOUND) '
                         'and the message names the nominated catch pose. This '
                         'is the ONLY pre-throw gate that bounds B laterally '
                         'under 8b, where the positioning move commands the '
                         'pre-tilt pose at the throw site A and never judges B. '
                         '(b) POSITIONING: trajectory/go_to_pose REFUSED the '
    ...
    # ── The UNIFIED 7-DoF codes. They were minted between 2026-09-05 and
    ...
    'REJECTED_CYCLE_PLAN': 'trajectory_node refused BEFORE planning, and the '
                           ...
                           'REPLAN_BUDGET are properties of the SERVICE\'s '
                           'acceptance state — a node in the wrong mode, not a '
                           'trajectory that cannot be flown. '
                           'HAND_BELOW_FLOOR is different and is the common one: '
                           'the hand is parked BELOW the planner\'s cup floor '
                           '(689.6 mm = 0.3162 rev) and the session-start lift '
                           'that should have fixed that did not land. Minted '
                           'before any solve, so it costs nothing — but note the '
                           'SILENT half it also closes: a seed between the floor '
                           'and -0.0485 rev is ACCEPTED by the QP and flies a '
                           '0.35 rev knot-1 step with the cup arcing PAST its '
                           'release site. CHAIN_SKEW means the chained release '
                           'and the cycle\'s scheduled one disagree by more than '
                           'one tick, i.e. the session re-scheduled under the '
                           'plan; the message quotes the skew',
    'REJECTED_PLAN_SERVICE': 'trajectory/plan_cycle was unavailable or did not '
                             'ack. ⚠ READ THE SUBCODE — the two halves are not '
                             'the same event. UNAVAILABLE = no request ever left '
                             'the coordinator, so NOTHING was planned and '
                             'nothing was commanded. UNACKED = the request WAS '
                             'dispatched and did not answer in time, and '
                             'plan_cycle installs on accept and replies '
                             'afterwards — so a plan MAY be installed and '
                             'streaming, and for a LAUNCH it will throw the '
                             'ball. ⚠ "in time" differs by path: the LAUNCH uses '
                             'the flat 2.0 s client wait, while a CHAINED extend '
                             'uses `min(sent + 2.0, deadline - 0.160)` — the '
                             'supersede deadline less the margin the hold\'s own '
                             'work needs — because the client wait knows nothing '
                             'about that plan\'s cliff and expired 0.8 s past it '
                             'at the shipped beat. The message says which bound '
                             'bit. It also carries the verdict of the '
                             'trajectory/hold the coordinator issued. Treat the '
                             'machine as having moved',
    'REJECTED_UNIFIED_AIM_UNSUPPORTED': 'a DISPLACED catch_position on a UNIFIED '
                                        'goal, refused at ACCEPTANCE (|B-A| > '
                                        '5.0 mm from the LIVE commanded xy). The '
                                        'unified launch throws VERTICALLY by '
                                        'construction — the request pins '
                                        'throw_target == throw_site — so there '
                                        'is no seam at which a displacement '
                                        'could become an aim, and until '
                                        '2026-09-07 such a goal was ACCEPTED and '
                                        'the LEGACY preamble tilted the platform '
                                        'to an aim the plan then silently undid. '
                                        'Set catch_position to the pre-throw xy '
                                        '(the message quotes it), or run the '
                                        'goal with unified_cycle false. Aimed '
                                        'unified cycles need the aim-authority '
                                        're-derivation against MAX_TILT_DEG '
                                        'first — do NOT widen the 5 mm tolerance',
    'REJECTED_BEAT_TOO_SHORT': 'UH-7a, UNIFIED ONLY, refused at ACCEPTANCE. TWO '
                               'different gates share this code and the message '
                               'names which one bound. (1) THE BEAT — the dwell '
                               'is under the chain floor, itself the larger of '
                               '(a) the carry `dwell - dt` >= '
                               '_UNIFIED_EXTEND_LEAD_S 0.60 (the extend is asked '
                               'for after the live catch and must install before '
                               'the supersede deadline, which sits ONE KNOT '
                               'before the terminal release — so the carry is '
                               'the dwell LESS that knot, not the dwell) and (b) '
                               'the next cycle\'s lead: it cannot start before '
                               '`catch + max(verdict 0.560, extend 0.600)` and '
                               'then needs 0.160 s of preamble plus one 0.040 s '
                               'tick — 0.800 s, and (b) is what binds. (2) '
                               'CYCLE 1 — throw_delay_s under 0.866 s: the first '
                               'cycle\'s release is DERIVED (now + throw_delay) '
                               'while the LAUNCH lands its release at install + '
                               '0.6, so a short delay makes the ball leave LATE '
                               'against the 0.5 s grace and cycle 1 aborts '
                               'NO_RELEASE before any chain exists. Both are '
                               'PLUMBING floors, not the planner\'s: whether the '
                               'resulting beat is FLYABLE is answered per cycle '
                               'by the QP, and a refusal there is safe because '
                               'the chain holds the machine. NOTE the legacy '
                               'session floor `dwell >= throw_delay + 0.120` '
                               'still applies, so a goal at the cycle-1 delay '
                               'floor needs dwell >= 0.986 s — that, not 0.800, '
                               'is a unified session\'s real minimum dwell',
    'STOPPED_CHAIN_REFUSED': 'UH-7a, a SESSION terminal (not a cycle reject): '
                             'the planner refused the ring\'s next STEADY '
                             'window, the coordinator\'s fall-back LANDING '
                             'installed, the machine CAUGHT the ball in the air '
                             'and settled — and the session then stopped by '
                             'name instead of continuing. It stops on purpose: '
                             'continuing is physically safe but silently '
                             'reverts the beat to the settle-plus-relaunch '
                             'cadence for the rest of the sitting, and a corpus '
                             'with two cadences in it is a corpus with two '
                             'machines in it. The detail is the PLANNER\'s own '
                             'composed refusal (nested parentheses, so there is '
                             'no flat subcode to match on) — read which layer '
                             'said no out of it. The action is to RAISE '
                             'dwell_time_s (or the session limits, if the '
                             'refusal is LIMIT_*) and re-run; the throws before '
                             'the stop are valid data at the commanded beat',
    # ── The auto-reload interlude's own SESSION terminals (§ 3.9). They were
    # minted 2026-08-11 and went unhinted until the completeness test in
    # tests/ros/test_outcome_detail.py found them on 2026-09-07 — which is the
    # point of that test.
    'STOPPED_RELOAD_BUDGET': 'the session spent its reload budget: max_reloads '
                             'BallButler balls have already been delivered for '
                             'this goal and another REJECTED_NO_BALL arrived. '
                             'NOT a fault — a fence. Every reload is a real ball '
                             'that left BB, so the budget is what stops an empty '
                             'cup from consuming the whole magazine. Clear the '
                             'floor, reload the machine, and raise max_reloads '
                             'only if you meant to',
    'STOPPED_FLOOR_CLEAR_REQUIRED': 'floor_pause_every balls have reached the '
                                    'FLOOR this session and the operator is '
                                    'being asked to clear them before another '
                                    'is thrown. Also NOT a fault: the count is '
                                    'charged per reload ATTEMPT, so a '
                                    'multi-attempt interlude advances it by more '
                                    'than one. Pick the balls up and re-issue '
                                    'the goal',
    'STOPPED_RELOAD_FAILED': 'the node reported a failed interlude with no code '
                             'of its own — NEVER EXPECTED, and it still stops, '
                             'because an unnamed failure is still a failure and '
                             'continuing would stroke over a cup nobody proved '
                             'has a ball. A firing is a defect report: the node '
                             'should have named which rung refused '
                             '(STOPPED_BB_NOT_READY / STOPPED_BB_UNVERIFIED / '
                             'STOPPED_SENSOR_UNKNOWN / STOPPED_CUP_NOT_EMPTY / '
                             'STOPPED_BALL_EVIDENCE_DISABLED / '
                             'STOPPED_RECENTRE_FAILED). Capture the console',
    'STOPPED_CHAIN_LOST': 'UH-7a, a SESSION terminal (not a cycle reject): the '
                          'ring lost its next window ENTIRELY — the STEADY '
                          'failed, and the LANDING fall-back was either refused '
                          'too or had no time left to be tried — so the '
                          'coordinator HELD the machine and the session stopped. '
                          'Distinct from STOPPED_CHAIN_REFUSED, where a window '
                          'DID install and the ball was caught by a plan: here '
                          'nothing was chained and the stop is what a '
                          'trajectory/hold left behind. The CYCLE keeps its own '
                          'verdict — a catch that happened still counts — '
                          'because the failure is the ring\'s, not the throw\'s. '
                          'The detail is `NO_WINDOW: <refusals> — <hold '
                          'verdict>`: read the HOLD VERDICT first, because a '
                          'plan with a release still ahead throws again on its '
                          'own and "trajectory/hold UNAVAILABLE" there means a '
                          'stroke is still coming. Then the refusals: LIMIT_* = '
                          'the beat is not flyable at these limits (raise '
                          'dwell_time_s or the limits), UNACKED = the planner '
                          'wedged and the ladder gave up on the supersede '
                          'deadline rather than on the client wait',
    'REJECTED_CHAIN_LOST': 'UH-7a: a standing STEADY chain could not be '
                           'extended, the LANDING fall-back could not be planned '
                           'either, and the machine was HELD. The message '
                           'carries BOTH refusals and the hold\'s own verdict — '
                           'read the hold verdict first, because a plan that '
                           'ENDS AT A RELEASE throws again on its own and '
                           '"trajectory/hold UNAVAILABLE" there means a stroke '
                           'is still coming. Then read the two refusals: '
                           'LIMIT_* means the beat is not flyable at these '
                           'limits (raise dwell_time_s or the limits), NO_CYCLE '
                           '/ STALE_STATE means the chain lost its seam',
}
