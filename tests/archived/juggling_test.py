...
# Active Z from config
ACTIVE_Z = float(hw.JB_OP_DEFAULT_ACTIVE_Z_MM)  # 170.0
...
def build_shuttle_targets(
    ...
    left_pos = [-SHUTTLE_RADIUS_MM, 0.0, ACTIVE_Z]
    left_quat = rotvec_to_quat(0.0, +tilt_rad)

    right_pos = [+SHUTTLE_RADIUS_MM, 0.0, ACTIVE_Z]
...
def test_star(
    ...
    print(f"  Speed scale:     {speed_scale}")
    print(f"  Move duration:   {actual_dur:.1f}s (base {move_duration:.1f}s)")
    print(f"  Points:          {n_moves - 1} (5-pointed star + return to active)")
...
def build_juggling_test_trajectories(
    ...
    params = DynamicsParams.from_config()
    active_pose = np.array([0.0, 0.0, ACTIVE_Z, 0.0, 0.0, 0.0])
    zeros6 = np.zeros(6)
