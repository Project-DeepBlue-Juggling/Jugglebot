...
Phase 4 hardware tests (free platform, <=25% speed):
  T1. Small move from active pose (10mm Z, 1.0s, 25% speed)
    ...
    *,
    ...
    meta = {
        ...
        'kinematic_limits': {
            ...
            'condition_limit': 2.0 * cond_reference,
            'condition_reference': cond_reference,
        },
        ...
        # Geometry snapshot
        'geometry': {
            'active_z_mm': float(hw.JB_OP_DEFAULT_ACTIVE_Z_MM),
...
def make_rest_to_rest(end_pose, duration, speed_scale=DEFAULT_SPEED_SCALE,
    ...
    start_pose : (6,) array or None — defaults to active pose (zeros)
    """
    zeros6 = np.zeros(6)
...
def move_to_active(harness: PlatformTestHarness, geom: StewartGeometry,
    ...
    from jugglebot.motion.dynamics import gravity_to_motor_torques
    active_torques = gravity_to_motor_torques(
...
def test_small_move(harness: PlatformTestHarness,
    ...
    """
    print("\n" + "=" * 60)
    print("T1: Small move from active pose (+10mm Z)")
    print("=" * 60)
    ...
    print_feasibility('Active -> +10mm Z', result_fwd)
    ...
    print_feasibility('+10mm Z -> Active', result_ret)
    ...
    print(f"\n  {'PASS' if all_pass else 'FAIL'}: T1 Small move from active pose")
    return all_pass
...
def test_graduated(harness: PlatformTestHarness,
    ...
    Moves (each from active pose, return to active pose):
      a) +20mm Z
...
def test_multi_pose(harness: PlatformTestHarness,
    ...
    Chains 5 poses in sequence (start from active pose, return to active pose):
      1. (0, 0, 30) mm
      2. (20, -10, 20) mm
...
TESTS = {
    'small_move':  ('T1: Small move from active pose (10mm Z)',
                    test_small_move),
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Tests available:
  small_move    T1: Small move from active pose (10mm Z, 25%% speed)
