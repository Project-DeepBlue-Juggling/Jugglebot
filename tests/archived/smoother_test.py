...
Tests (graduated risk):
  S1. Static hold — reset smoother to active pose, hold for 2s.  Platform should
...
ACTIVE_POSE = np.array([0, 0, hw.JB_OP_DEFAULT_ACTIVE_Z_MM, 0, 0, 0],
                        dtype=float)
...
def move_to_active(harness: PlatformTestHarness, geom: StewartGeometry,
                 params: DynamicsParams):
    """Move to active pose using TRAP_TRAJ (safe ramped transition)."""
    active_pos = ACTIVE_POSE[:3]
    active_rot = rotvec_to_rot_matrix(ACTIVE_POSE[3:6])
    active_raw = pose_to_raw_positions(active_pos, active_rot, geom)
    active_torques = gravity_to_motor_torques(active_pos, active_rot, geom, params)

    harness.enter_trap_traj_mode_all(vel_limit=1.5, acc_limit=5.0,
                                     dec_limit=5.0)
    for i, axis_id in enumerate(LEG_AXES):
        tor_int = int(round(-active_torques[i] * LEG_TOR_FF_SCALE))
        tor_int = max(-32767, min(32767, tor_int))
        harness.send(encode_set_input_pos(
            axis_id, active_raw[i], vel_ff=0, torque_ff=tor_int))

    harness.wait_for_all_trajectories_done(timeout_s=15.0)
    harness.poll_for(1.0)
    print("  At active pose.")
...
def safe_shutdown(harness: PlatformTestHarness, geom: StewartGeometry,
                  params: DynamicsParams):
    """Return to active pose, stow, then idle all axes.

    Called on normal completion and on error.  Never idles axes
    unless the platform is in the stowed position.
    """
    try:
        move_to_active(harness, geom, params)
    except Exception as e:
        print(f"  WARNING: Failed to return to active pose: {e}")
    try:
...
DWELL_AT_ACTIVE = 0.5     # seconds to dwell at active pose between tests
...
def preview_all_tests():
    ...
    Stitches S1-S5 end-to-end with C2-continuous return-to-active transitions
    ...
    return_boundaries = []  # (start_t, end_t) for return-to-active segments
    ...
    for name, targets, duration, desc, ff_enabled in tests_data:
        ...
        # Return to active pose (between tests and after the last test)
        return_start = t
        smoother.set_target(ACTIVE_POSE.copy(), t)
        return_dur = smoother._duration
        return_end = t + return_dur + DWELL_AT_ACTIVE
    ...
    # -- Stow segment: TRAP_TRAJ from active extensions to 0 rev --
    ...
    stow_start = t
    active_rot = rotvec_to_rot_matrix(ACTIVE_POSE[3:6])
    active_ext = pose_to_leg_lengths(ACTIVE_POSE[:3], active_rot, geom)
    active_rev = extensions_mm_to_revs(active_ext, geom)
    # TRAP_TRAJ params matching stow_platform(): vel=1.5 rps, acc=5.0 rps2
    stow_vel = 1.5   # rev/s
    stow_acc = 5.0    # rev/s^2
    max_disp = np.max(np.abs(active_rev))  # revolutions to travel
    ...
    # Simulate: linearly interpolate extensions from active to 0 using
    ...
    while t < stow_end:
        ...
        ext_now = active_ext * (1.0 - frac)
        # Approximate pose: just interpolate Z linearly (stow is pure vertical)
        pose_now = ACTIVE_POSE.copy()
        pose_now[2] = ACTIVE_POSE[2] * (1.0 - frac)
    ...
    for row_idx, (title, arr, ylabel, use_leg) in enumerate(ROW_SPECS):
        ...
        # Return-to-active boundary markers (lighter blue)
...
def run_test(harness: PlatformTestHarness, test_name: str) -> bool:
    ...
    for t_time, pose in targets:
        smoother_copy = StreamSmoother(
            ...
            vel_limit_rps=hw.ODRIVE_TRAP_VEL_LIMIT_RPS,
            accel_limit_rps2=hw.ODRIVE_TRAP_ACC_LIMIT_RPS2)
        smoother_copy.reset(ACTIVE_POSE)
        smoother_copy.set_target(pose, 0.0)
        print(f"  Expected smoother duration (from active pose): "
              f"{smoother_copy._duration * 1000:.1f} ms")
        break  # only show first target

    # Confirm with operator
    interactive_pause("Platform is at active pose. Press Enter to execute...")
...
def run_dry_run(show_preview: bool):
    ...
    for test_name in test_names:
        ...
        if targets:
            ...
            print(f"    Smoother duration (from active pose): "
...
def main() -> int:
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Tests:
  static_hold   S1: Hold at active pose (2s, no targets)
    ...
    try:
        with harness:
            ...
            for test_name in tests_to_run:
                # Return to active pose between tests
                if test_name != tests_to_run[0]:
                    print("\n  Returning to active pose...")
                    move_to_active(harness, geom, params)
                    switch_to_passthrough(harness)
