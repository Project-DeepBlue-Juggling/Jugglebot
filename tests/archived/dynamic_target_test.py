...
Phase 7 hardware tests:
  DT1. Static target from active pose — simple zero-twist move
...
# Active Z for trajectory building (same as TrajectoryManager)
_ACTIVE_Z = float(hw.JB_OP_DEFAULT_ACTIVE_Z_MM)  # 170.0
...
def build_phase7_test_trajectories(
    ...
    active_pose = np.array([0.0, 0.0, _ACTIVE_Z, 0.0, 0.0, 0.0])
    zeros6 = np.zeros(6)
    ...
    # --- DT1: Static target from active pose ---
    dt1_dur = 1.0 / speed_scale
    trajs.append(('DT1: Active -> Z=200 (static)',
                  _make_traj(active_pose, [0, 0, 200], [0, 0, 0], dt1_dur)))
    trajs.append(('DT1: Z=200 -> Active (return)',
                  _make_traj([0, 0, 200, 0, 0, 0], [0, 0, _ACTIVE_Z],
                             [0, 0, 0], dt1_dur)))

    # --- DT2: Nonzero velocity + decelerate to stop ---
    dt2_dur = 1.0 / speed_scale
    dt2_vel = 30 * speed_scale
    dt2_outbound = _make_traj(active_pose, [0, 0, 190],
                              [0, 0, dt2_vel], dt2_dur)
    trajs.append(('DT2: Active -> Z=190 (vel=+Z)', dt2_outbound))
    ...
    # First target: active -> Z=200
    dt3_dur = 1.5 / speed_scale
    trajs.append(('DT3: Active -> Z=200 (1st target)',
                  _make_traj(active_pose, [0, 0, 200], [0, 0, 0], dt3_dur)))
    ...
    mgr_dt3.set_hold_pose(active_pose)
    t_sim3 = 100.0
    ...
    mgr_dt4.set_hold_pose(active_pose)
    t_sim4 = 100.0
    ...
    trajs.append(('DT5: Active -> Z=190 (feasible)',
                  _make_traj(active_pose, [0, 0, 190], [0, 0, 0], dt5_dur)))
    trajs.append(('DT5: Z=190 -> Active (return)',
                  _make_traj([0, 0, 190, 0, 0, 0], [0, 0, _ACTIVE_Z],
                             [0, 0, 0], dt5_dur)))
...
def move_to_active(harness: PlatformTestHarness,
                        geom: StewartGeometry,
                        params: DynamicsParams):
    """Move the physical platform to the active pose (Z=170mm).

    Dynamic target tests need the platform at the TrajectoryManager's
    active pose [0, 0, 170, 0, 0, 0] so that the manager's internal state
    matches the physical platform position.
    """
    active_pos = np.array([0.0, 0.0, float(hw.JB_OP_DEFAULT_ACTIVE_Z_MM)])
    _move_to_pose(harness, active_pos, geom, params,
                  f"At active pose (Z={hw.JB_OP_DEFAULT_ACTIVE_Z_MM}mm).")
...
def execute_with_dynamic_targets(
    ...
    ----------
    harness : PlatformTestHarness
    mgr : TrajectoryManager (should be in IDLE at active pose)
    geom : StewartGeometry
    params : DynamicsParams
    limits : WorkspaceLimits
...
def test_static_target(
    ...
    """DT1: Zero-twist target from active pose -> +30mm Z.

    Pass criteria: tracking error < threshold for speed scale.
    """
    print("\n" + "=" * 60)
    print("DT1: Static target from active pose (+30mm Z)")
    print("=" * 60)
    ...
    mgr.set_hold_pose(mgr.active_pose)
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Tests available:
  static      DT1: Static target from active pose (+30mm Z)
