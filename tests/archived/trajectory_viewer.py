...
Usage:
  python tools/trajectory_viewer.py --active
...
def build_test_trajectories(geom: StewartGeometry = None,
    ...
    # --- T1: Small move from active pose ---
    trajs.append(('T1: Active -> +10mm Z', rest_to_rest([0, 0, 10, 0, 0, 0], 1.0)))
    trajs.append(('T1: +10mm Z -> Active', rest_to_rest([0, 0, 0, 0, 0, 0], 1.0,
                                                          start_pose=[0, 0, 10, 0, 0, 0])))

    # --- T2: Graduated distance/tilt ---
    trajs.append(('T2a: Active -> +20mm Z',
                  rest_to_rest([0, 0, 20, 0, 0, 0], 1.0)))
    trajs.append(('T2a: +20mm Z -> Active',
                  rest_to_rest([0, 0, 0, 0, 0, 0], 1.0,
                               start_pose=[0, 0, 20, 0, 0, 0])))

    rx5 = np.deg2rad(5)
    trajs.append(('T2b: Active -> 5 deg tilt X',
                  rest_to_rest([0, 0, 0, rx5, 0, 0], 1.0)))
    trajs.append(('T2b: 5 deg tilt X -> Active',
                  rest_to_rest([0, 0, 0, 0, 0, 0], 1.0,
                               start_pose=[0, 0, 0, rx5, 0, 0])))

    trajs.append(('T2c: Active -> 20mm Z + 5 deg',
                  rest_to_rest([0, 0, 20, rx5, 0, 0], 1.5)))
    trajs.append(('T2c: 20mm Z + 5 deg -> Active',
                  rest_to_rest([0, 0, 0, 0, 0, 0], 1.5,
                               start_pose=[0, 0, 20, rx5, 0, 0])))

    trajs.append(('T2d: Active -> +40mm Z',
                  rest_to_rest([0, 0, 40, 0, 0, 0], 1.5)))
    trajs.append(('T2d: +40mm Z -> Active',
                  rest_to_rest([0, 0, 0, 0, 0, 0], 1.5,
    ...
    trajs.append(('T4: 30mm Z -> Active @ 12.5%',
                  create_trajectory(
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python tools/trajectory_viewer.py --active            Show active pose
  ...
  python tools/trajectory_viewer.py --phase7-sequence --speed-scale 0.3  Phase 7 tests
    ...
    group.add_argument('--active', action='store_true',
                       help='Display the platform at active pose')
    ...
    if args.active:
        show_pose(np.zeros(6), geom, title='Active Pose')
