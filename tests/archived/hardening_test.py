...
def build_workspace_boundary_trajectories(geom, speed_scale=0.25):
    ...
    # Large Z down (approaches lower extension limit — limited workspace below active pose)
...
def test_workspace_boundary(harness, geom, params, limits, speed_scale, dry_run=False):
    ...
    for label, traj in trajectories:
        ...
        # Return to active pose via TRAP_TRAJ before each trajectory (safe ramped move)
...
def test_fault_injection_static(harness, geom, params, limits, dry_run=False):
    ...
    if dry_run:
        ...
        print("  Procedure:")
        print("    1. Platform holds active pose")
    ...
    print("\n  Setup: Platform holding active pose with feedforward active.")
    print("  This test requires manual operator intervention.")
    print()
    print("  Procedure:")
    print("    1. Platform will hold at active pose")
    ...
    # Hold at active pose
    active_pos = pose_to_raw_positions(np.zeros(3), np.eye(3), geom)
    for i, axis_id in enumerate(LEG_AXES):
        harness.send(encode_set_input_pos(axis_id, active_pos[i], 0, 0))

    time.sleep(1.0)
    print("  Platform holding at active pose. All legs active.")
    print()
    ...
    try:
        while time.perf_counter() < t_endurance_end:
            ...
            for label, traj in trajectories:
                ...
                # Return to active pose via TRAP_TRAJ before each trajectory
                move_to_active(harness, geom, params)
                switch_to_passthrough(harness)
