...
from tests.ros._bridge_harness import (
    _build_paired_node, _teardown, _wait_until,
)
...
# ── R1 (c2) hand-lane arming choreography (no mastery latch) ──────────────────
# One hand master: the hand lane is ACTIVE whenever a frame carries HAS_HAND —
# there is no hand_source latch to read. These mirror the leg arm battery
# above (e.g. test_arm_rejected_on_u0_encoder_mismatch).
...
def test_arm_rejected_on_hand_u0_encoder_mismatch():
    """(a) hand u0 3 rev off the hand encoder ⇒ arm refused, naming the
    0.625 rev hand gate (quarter of MAX_DEVIATION_HAND_REV) — never the leg
    0.25. Arming anyway would hand the streamed lane a knot that trips the
    hand deviation guard / lead clamp at the arm edge."""
    teensy, client, node = _node()
    try:
        _bring_link_and_telem_up(teensy, node, leg_pos=0.1)
...
def test_arm_with_hand_knots_runs_hand_closed_loop_preamble_before_mpc_active():
    """(b) a hand-bearing frame ⇒ the arm fold sends SET_AXIS_STATE(6,
    CLOSED_LOOP) then SET_CONTROLLER_MODE(6, POSITION/PASSTHROUGH) BEFORE
    mpc_active rises — the single arm gate is the ONLY place this preamble
    runs (a hand left IDLE would silently swallow the streamed lane; the
    idle-axis trap, logbook/2026-09-04-fw17-hand-sitting-unflashed-idle-axis.md)."""
...
def test_arm_without_hand_knots_skips_the_hand_preamble():
    """A leg-only frame (no HAS_HAND) never touches the hand ODrive — the
    preamble is conditioned on the frame, not run unconditionally on every
    arm."""
    from teensy_link import RpcMethod, RpcStatus
    teensy, client, node = _node()
    try:
        ...
        assert resp.success is True, resp.message
        assert node._mpc_active is True
        assert ax_calls == []            # no hand preamble without a hand knot
    finally:
        _teardown(teensy, client, node)
