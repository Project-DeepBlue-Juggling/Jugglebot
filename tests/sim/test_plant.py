...
class TestPlantActivePose:
    """Plant state at active pose is correct."""

    def test_active_state(self, plant):
...
class TestPlantReset:
    """Reset returns the platform to active pose."""

    def test_reset_to_active(self, plant):
        # First move away from active pose
...
class TestTelemetryLogger:
    ...
    def test_periodic_flush_and_load(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for i in range(120):
                rec = record_from_arrays(
                    ...
                    actual_twist=np.zeros(6),
                    cmd_extensions=np.zeros(6),
                    actual_extensions=np.zeros(6),
                )
                logger.append(rec)
            ...
            # load() returns the full history from CSV
            all_records = logger.load()
            assert len(all_records) == 120


class TestCommandVelMmS:
    """command() accepts optional vel_mm_s kwarg."""

    def test_command_accepts_vel_mm_s(self):
        """command() with vel_mm_s does not raise."""
        p = MuJoCoPlant()
        cmd = p.get_state().leg_extensions_mm
        p.command(cmd, vel_mm_s=np.zeros(6))
        p.step(CONTROL_DT)

    def test_command_without_vel_mm_s(self):
        """command() without vel_mm_s still works (backward-compatible)."""
        p = MuJoCoPlant()
        cmd = p.get_state().leg_extensions_mm
        p.command(cmd)
        p.step(CONTROL_DT)
