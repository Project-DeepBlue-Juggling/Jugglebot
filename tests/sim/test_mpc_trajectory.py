...
Tests:
  - T1 (linear translation): arrival error < 3 mm; returns to active pose
...
class TestT1LinearTranslation:
    """T1: active pose -> z+50mm -> active pose, 1s each segment."""
    ...
    def test_returns_to_active(self, plant):
        """Platform returns close to final waypoint after trajectory completes."""
        plant.reset()
        mpc = _create_mpc(plant)
        waypoints, duration = make_T1()
        ...
        pos_err = np.linalg.norm(final_pose[:3] - final_target[:3])
        ori_err = np.degrees(np.linalg.norm(final_pose[3:] - final_target[3:]))
...
class TestVelocityTracking:
    ...
    def test_zero_velocity_at_waypoint(self, plant):
        ...
        The MPC transitions from "approach z50" to "return to active pose" at the waypoint
...
class TestT5GrandTour:
    """T5: Grand tour — 10 poses spanning the workspace, all 6 DoFs."""

    def test_all_waypoints_reached(self, plant):
        """Platform arrives within 5 mm of each waypoint at deadline."""
        plant.reset()
        mpc = _create_mpc(plant)
        waypoints, duration = make_T5()
        ...
        for e in errors:
            assert e['pos_err_mm'] < 5.0, (
                f"T5 waypoint {e['waypoint_idx']} (t={e['arrival_time']:.1f}s) "
                f"pos error {e['pos_err_mm']:.2f} mm > 5.0 mm"
            )

    def test_no_solver_failures(self, plant):
        """No solver failures during T5 (after warm-up)."""
        plant.reset()
        mpc = _create_mpc(plant)
        waypoints, duration = make_T5()
        ...
        assert len(failures) == 0, (
            f"{len(failures)} solver failures during T5 "
            f"(first: {failures[0]['status'] if failures else 'N/A'})"
        )


class TestT6ExtremeWorkspace:
    """T6: Extreme tour — big XY sweeps, ~120 mm lateral, Z 60-260 mm."""

    def test_all_waypoints_reached(self, plant):
        """Platform arrives within 5 mm of each waypoint at deadline."""
        plant.reset()
        mpc = _create_mpc(plant)
        waypoints, duration = make_T6()
    ...
    def test_stroke_limits(self, plant):
        """Commanded extensions stay within stroke bounds during T6."""
        plant.reset()
        mpc = _create_mpc(plant)
        waypoints, duration = make_T6()
        ...
        for s in states:
            assert np.all(s['cmd'] >= -0.5), \
                f"T6 extension below 0: {np.min(s['cmd']):.2f}"
            assert np.all(s['cmd'] <= 280.5), \
                f"T6 extension above 280: {np.max(s['cmd']):.2f}"


class TestFeasibilityFromMotion:
    """MPC replanning from nonzero initial velocity (mid-motion target switch).

    Starts tracking T2 circular orbit, then switches to a static target
    while the platform is moving. Validates convergence to the new target.
    """

    def test_converges_after_midmotion_switch(self, plant):
        """Platform reaches new static target after switching mid-orbit."""
        plant.reset()
        mpc = _create_mpc(plant)
        waypoints_t2, _ = make_T2()

        # Phase 1: run T2 for 2.0s to build up orbital velocity
        n_phase1 = int(2.0 / CONTROL_DT)
        idx = 0
        for _ in range(n_phase1):
            state = plant.get_state()
            while (idx + 1 < len(waypoints_t2)
                   and state.time >= waypoints_t2[idx][1]):
                idx += 1
            target_pose, arrival_time = waypoints_t2[idx]
            current_pose = np.concatenate([state.platform_pos_mm, state.platform_rot])
            ref_events = flat_target_to_events(
                current_pose, state.platform_twist, target_pose, state.time,
                arrival_time=arrival_time)
            cmd, _, _ = mpc.solve(state, target_pose, ref_events=ref_events)
            plant.command(cmd)
            plant.step(CONTROL_DT)

        # Verify platform has nonzero velocity at switchover
        switch_state = plant.get_state()
        switch_speed = np.linalg.norm(switch_state.platform_twist[:3])
        assert switch_speed > 10.0, (
            f"Platform speed at switch {switch_speed:.1f} mm/s too low; "
            f"test invalid (expected >10 mm/s from orbital motion)"
        )

        # Phase 2: switch to static target and run for 1.5s
        static_target = np.array([0.0, 0.0, 50.0, 0.0, 0.0, 0.0])
        n_phase2 = int(1.5 / CONTROL_DT)
        final_state = None
        for _ in range(n_phase2):
            state = plant.get_state()
            cmd, _, _ = mpc.solve(state, static_target)
            plant.command(cmd)
            plant.step(CONTROL_DT)
            final_state = state

        # Platform should converge to the static target
        final_pose = np.concatenate([final_state.platform_pos_mm,
                                     final_state.platform_rot])
        pos_err = np.linalg.norm(final_pose[:3] - static_target[:3])
        ori_err = np.degrees(np.linalg.norm(final_pose[3:] - static_target[3:]))

        assert pos_err < 2.0, (
            f"After mid-motion switch: pos error {pos_err:.2f} mm > 2.0 mm"
        )
        assert ori_err < 1.0, (
            f"After mid-motion switch: ori error {ori_err:.3f} deg > 1.0 deg"
        )
